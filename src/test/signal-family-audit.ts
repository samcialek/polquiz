...
function collectQuestions(): AnyQuestion[] {
  return REPRESENTATIVE_QUESTIONS.filter((q: AnyQuestion) => {
    if (TARGET_PROMPTS.has(q.promptShort)) return true;
    return (q.touchProfile || []).some((t: any) => TARGET_TOUCH_TYPES.has(t.touchType));
  });
}

function summarizeTouchProfile(q: AnyQuestion): string {
  const rows = (q.touchProfile || []).map((t: any) => {
...
function summarizeOptions(q: AnyQuestion): string {
  const opts = q.options || [];
  if (!opts.length) return '<p class="muted">No explicit options array.</p>';
  return `<ul>${opts.map((o: string) => `<li><code>${esc(o)}</code></li>`).join('')}</ul>`;
}

function summarizeEvidence(q: AnyQuestion): string {
  if (q.optionEvidence) {
...
function notes(q: AnyQuestion): string[] {
  const out: string[] = [];
  ...
  const salienceTouches = (q.touchProfile || []).filter((t: any) => t.role === 'salience');
  if (salienceTouches.length) {
    out.push(`Includes explicit salience signal on: ${salienceTouches.map((t: any) => `${t.node} (${t.weight})`).join(', ')}.`);
  }
  return out;
}
