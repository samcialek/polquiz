...
// ---------------------------------------------------------------------------
// Persona (same shape as Layer 2)
// ---------------------------------------------------------------------------

interface PersonaContinuous { pos: number; sal: number; }
interface PersonaCategorical { probs: CategoricalDist; sal: number; }

interface Persona {
  archId: string;
  archName: string;
  continuous: Partial<Record<ContinuousNodeId, PersonaContinuous>>;
  categorical: Partial<Record<CategoricalNodeId, PersonaCategorical>>;
}
...
function buildPersona(arch: Archetype): Persona {
  ...
  for (const [nid, t] of Object.entries(arch.nodes)) {
    if (t.kind === "continuous") {
      persona.continuous[nid as ContinuousNodeId] = { pos: t.pos, sal: t.sal ?? 0 };
    } else {
      persona.categorical[nid as CategoricalNodeId] = { probs: t.probs, sal: t.sal ?? 0 };
    }
  }
  return persona;
}
...
function pickSamplePersonas(count: number): Persona[] {
  const active = ARCHETYPES.filter(a => a.active !== false);
  if (count >= active.length) return active.map(buildPersona);
  ...
  for (let i = 0; i < active.length && picks.length < count; i++) {
    if (!picks.includes(active[i]!)) picks.push(active[i]!);
  }
...
// ---------------------------------------------------------------------------
// Drive one persona through the browser-loaded PrismEngine
// ---------------------------------------------------------------------------
...
interface BrowserRunResult {
  ...
  top1Name: string;
  top1Distance: number;
  top3: { id: string; name: string; distance: number }[];
  targetRank: number;
  questionsAnswered: number;
  respondentStateShape: "nested" | "flat" | "missing";
  hasExpectedPos: boolean;
  hasSalience: boolean;
  hasCatDist: boolean;
}

async function runPersonaInBrowser(page: Page, persona: Persona): Promise<BrowserRunResult> {
  // Evaluate the full persona-replay loop in the browser context
  const result = await page.evaluate(async (personaJson) => {
    const persona = JSON.parse(personaJson);
    const PE = (window as unknown as { PrismEngine: Record<string, unknown> }).PrismEngine;
    type OptionEvidence = { continuous?: Record<string, { pos?: number[]; sal?: number[] }>; categorical?: Record<string, { cat?: number[]; sal?: number[] }> };
    type BucketMap = { continuous?: Record<string, number>; categorical?: Record<string, number[]> };

    // Persona scorers (same logic as Layer 2)
    function scoreOptionEvidence(ev: OptionEvidence | undefined): number {
      if (!ev) return 0;
      let score = 0, count = 0;
      if (ev.continuous) {
        for (const [nid, upd] of Object.entries(ev.continuous)) {
          ...
          if (!t || !upd) continue;
          if (upd.pos) { score += upd.pos[t.pos - 1] ?? 0.2; count++; }
          if (upd.sal) { score += upd.sal[t.sal] ?? 0.25; count++; }
        }
      }
      if (ev.categorical) {
        for (const [nid, upd] of Object.entries(ev.categorical)) {
          ...
          if (upd.cat) {
            let dot = 0;
            for (let i = 0; i < 6; i++) dot += (upd.cat[i] ?? 0) * (t.probs[i] ?? 0);
            score += dot * 6;
            count++;
          }
          if (upd.sal) { score += upd.sal[t.sal] ?? 0.25; count++; }
    ...
    function scoreBucketMap(map: BucketMap | undefined): number {
      if (!map) return 0;
      let score = 0, count = 0;
      if (map.continuous) {
        for (const [nid, signal] of Object.entries(map.continuous)) {
          const t = persona.continuous[nid];
          if (!t || signal === undefined) continue;
      ...
      if (map.categorical) {
        for (const [nid, catDist] of Object.entries(map.categorical)) {
          const t = persona.categorical[nid];
          if (!t || !catDist) continue;
          let dot = 0;
          for (let i = 0; i < 6; i++) dot += (catDist[i] ?? 0) * (t.probs[i] ?? 0);
          score += dot * 6;
          count++;
        }
      }
    ...
    type QuestionDef = {
      ...
      allocationMap?: Record<string, BucketMap>;
      rankingMap?: Record<string, BucketMap>;
      pairMaps?: Record<string, Record<string, BucketMap>>;
      bestWorstMap?: Record<string, BucketMap>;
    };
    type QuizQuestion = { id: number; options?: string[] };

    function decideAnswer(q: QuestionDef, qq: QuizQuestion): unknown {
      switch (q.uiType) {
        case "single_choice":
        case "multi": {
          const opts = Object.keys(q.optionEvidence ?? {});
          if (opts.length === 0) return qq.options?.[0] ?? "unknown";
        ...
        case "allocation": {
          ...
          if (total === 0) {
            ...
            for (const b of buckets) alloc[b] = share;
          } else {
            for (const { b, s } of scores) alloc[b] = Math.round((s / total) * 100);
            const sum = Object.values(alloc).reduce((a, b) => a + b, 0);
            if (sum !== 100 && buckets.length > 0) alloc[buckets[0]!] = (alloc[buckets[0]!] ?? 0) + (100 - sum);
          }
          return alloc;
        }
        case "ranking": {
        ...
        case "pairwise": {
          ...
          for (const [pairId, pair] of Object.entries(q.pairMaps ?? {})) {
            ...
            for (const c of choices) {
              ...
              if (s > bestScore) { bestScore = s; bestChoice = c; }
            }
        ...
        case "priority_sort": {
          ...
          return {
            supportHigh: scored.slice(0, q1).map(x => x.i),
            supportMid:  scored.slice(q1, q2).map(x => x.i),
            neutral:     scored.slice(q2, q3).map(x => x.i),
            opposeHigh:  scored.slice(q3).map(x => x.i),
          };
        }
    ...
    let steps = 0;
    const MAX = 120;
    while (!(PE.isComplete as () => boolean)() && steps < MAX) {
      steps++;
      const qq = (PE.getNextQuestion as () => QuizQuestion | null)();
      if (!qq) break;
      const q = (PE.getQuestionDef as (id: number) => QuestionDef | null)(qq.id);
      if (!q) { (PE.submitAnswer as (id: number, a: unknown) => void)(qq.id, qq.options?.[0] ?? "unknown"); continue; }
      const answer = decideAnswer(q, qq);
      (PE.submitAnswer as (id: number, a: unknown) => void)(qq.id, answer);
    }

    const results = (PE.getResults as () => { match: { id: string; name: string; distance: number }; top3: { id: string; name: string; distance: number }[]; questionsAnswered: number })();
    const respondentState = (PE.getRespondentState as () => Record<string, unknown> | null)();

    // Shape check
    let shape: "nested" | "flat" | "missing" = "missing";
    let hasExpectedPos = false, hasSalience = false, hasCatDist = false;
    if (respondentState) {
      const cont = (respondentState as { continuous?: Record<string, { expectedPos?: number; salience?: number }> }).continuous;
      const cat = (respondentState as { categorical?: Record<string, { catDist?: number[] }> }).categorical;
      ...
      } else if (Object.keys(respondentState).length > 0 && Object.values(respondentState).some(v => typeof v === "object" && v !== null && ("expectedPos" in (v as object) || "pos" in (v as object)))) {
        shape = "flat";
      }
...
async function main() {
  ...
  await fs.writeFile(
    path.join(outDir, "browser-e2e.json"),
    JSON.stringify({
