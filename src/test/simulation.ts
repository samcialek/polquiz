...
function scoreOptionEvidence(
  ...
  if (!evidence) return 1; // neutral — no evidence
...
function simulateArchetype(
  target: Archetype,
  archetypes: Archetype[],
  questions: QuestionDef[],
  maxQuestions: number = 65
): SimResult {
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    if (result.rank <= 3) top3Count++;
    if (result.rank <= 5) top5Count++;
    totalQuestions += result.questionsAnswered;

    const status = result.correct ? "OK" : `MISS(rank=${result.rank})`;
    const progress = `[${i + 1}/${targets.length}]`;
    console.log(
      `${progress} ${target.id} ${target.name.padEnd(35)} → ${status.padEnd(15)} ` +
    ...
    if (!result.correct) {
      console.log(
        `         Got: ${result.resultId} ${result.resultName}`
      );
      console.log(
        `         Top5: ${result.top5.join(", ")}`
      );
    }
  }
  ...
  if (misses.length > 0) {
    ...
    for (const [pair, count] of sorted.slice(0, 20)) {
      ...
      console.log(
        `  ${fromId} ${from?.name?.padEnd(30) ?? ""} → ${toId} ${to?.name?.padEnd(30) ?? ""} (×${count})`
      );
    }
    console.log();
    ...
    for (const r of worstPerformers) {
      ...
      console.log(
        `  ${r.archetypeId} ${r.archetypeName.padEnd(35)} rank=${r.rank} (got ${r.resultId} ${r.resultName})`
      );
    }
  }
  ...
  for (const tier of tiers) {
    ...
    console.log(
      `  ${tier.padEnd(8)} ${tierCorrect}/${tierResults.length} = ${(tierCorrect / tierResults.length * 100).toFixed(1)}%`
    );
  }
