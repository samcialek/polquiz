...
function buildPersona(arch: Archetype): Persona {
  const persona: Persona = { archId: arch.id, archName: arch.name, continuous: {}, categorical: {} };
  for (const [nid, t] of Object.entries(arch.nodes)) {
    if (t.kind === "continuous") persona.continuous[nid as ContinuousNodeId] = { pos: t.pos, sal: t.sal ?? 0 };
    else persona.categorical[nid as CategoricalNodeId] = { probs: t.probs, sal: t.sal ?? 0 };
  }
  return persona;
}
...
async function main() {
  ...
  const picks = [
    ...
    active.find(a => a.id === "060")!, // Hinge Citizen (moderate)
  ].filter(Boolean);

  const perPersona: { archId: string; archName: string; top1Id: string; matRespondent: number | null; matCentroid: number; nodeBarCount: number }[] = [];
  ...
  for (const arch of picks) {
    ...
    const driveResult = await page.evaluate(async (pJson) => {
      ...
      const scoreOE = (ev: OE | undefined): number => {
        if (!ev) return 0;
        let s = 0, c = 0;
        if (ev.continuous) for (const [nid, upd] of Object.entries(ev.continuous)) { const t = persona.continuous[nid]; if (!t || !upd) continue; if (upd.pos) { s += upd.pos[t.pos - 1] ?? 0.2; c++; } if (upd.sal) { s += upd.sal[t.sal] ?? 0.25; c++; } }
        if (ev.categorical) for (const [nid, upd] of Object.entries(ev.categorical)) { const t = persona.categorical[nid]; if (!t || !upd) continue; if (upd.cat) { let d = 0; for (let i = 0; i < 6; i++) d += (upd.cat[i] ?? 0) * (t.probs[i] ?? 0); s += d * 6; c++; } if (upd.sal) { s += upd.sal[t.sal] ?? 0.25; c++; } }
        return c > 0 ? s / c : 0;
      };
      const scoreBM = (m: BM | undefined): number => {
        if (!m) return 0;
        let s = 0, c = 0;
        if (m.continuous) for (const [nid, sig] of Object.entries(m.continuous)) { const t = persona.continuous[nid]; if (!t || sig === undefined) continue; s += sig * ((t.pos - 3) / 2); c++; }
        if (m.categorical) for (const [nid, cd] of Object.entries(m.categorical)) { const t = persona.categorical[nid]; if (!t || !cd) continue; let d = 0; for (let i = 0; i < 6; i++) d += (cd[i] ?? 0) * (t.probs[i] ?? 0); s += d * 6; c++; }
        return c > 0 ? s / c : 0;
      };
      ...
      const decide = (q: Q, qq: { id: number; options?: string[] }): unknown => {
        switch (q.uiType) {
          case "single_choice": case "multi": { const o = Object.keys(q.optionEvidence ?? {}); if (!o.length) return qq.options?.[0] ?? "unknown"; let b = o[0]!, bs = -Infinity; for (const x of o) { const s = scoreOE(q.optionEvidence![x]); if (s > bs) { bs = s; b = x; } } return b; }
          case "slider": { const o = Object.keys(q.sliderMap ?? {}); if (!o.length) return 50; let b = o[0]!, bs = -Infinity; for (const x of o) { const s = scoreOE(q.sliderMap![x]); if (s > bs) { bs = s; b = x; } } const p = b.split("-").map(Number); return Math.floor(((p[0] ?? 0) + (p[1] ?? 100)) / 2); }
          case "allocation": { const o = Object.keys(q.allocationMap ?? {}); if (!o.length) return {}; const sc = o.map(b => ({ b, s: Math.max(0, scoreBM(q.allocationMap![b]) + 0.5) })); const t = sc.reduce((a, b) => a + b.s, 0); const a: Record<string, number> = {}; if (t === 0) { const sh = Math.floor(100 / o.length); for (const b of o) a[b] = sh; } else { for (const { b, s } of sc) a[b] = Math.round((s / t) * 100); const sum = Object.values(a).reduce((x, y) => x + y, 0); if (sum !== 100) a[o[0]!] = (a[o[0]!] ?? 0) + (100 - sum); } return a; }
          case "ranking": { const o = Object.keys(q.rankingMap ?? {}); if (!o.length) return []; const sc = o.map(i => ({ i, s: scoreBM(q.rankingMap![i]) })); sc.sort((x, y) => y.s - x.s); return sc.map(x => x.i); }
          case "pairwise": { const a: Record<string, string> = {}; for (const [pid, p] of Object.entries(q.pairMaps ?? {})) { const c = Object.keys(p); if (!c.length) continue; let b = c[0]!, bs = -Infinity; for (const x of c) { const s = scoreBM(p[x]); if (s > bs) { bs = s; b = x; } } a[pid] = b; } return a; }
          case "best_worst": { const o = Object.keys(q.bestWorstMap ?? {}); if (!o.length) return { best: "", worst: "" }; const sc = o.map(i => ({ i, s: scoreBM(q.bestWorstMap![i]) })); sc.sort((x, y) => y.s - x.s); return { best: sc[0]!.i, worst: sc[sc.length - 1]!.i }; }
          case "priority_sort": { const o = Object.keys(q.rankingMap ?? {}); if (!o.length) return { supportHigh: [], supportMid: [], neutral: [], opposeHigh: [] }; const sc = o.map(i => ({ i, s: scoreBM(q.rankingMap![i]) })); sc.sort((x, y) => y.s - x.s); const n = sc.length; const q1 = Math.ceil(n / 4), q2 = Math.ceil(n / 2), q3 = Math.ceil((3 * n) / 4); return { supportHigh: sc.slice(0, q1).map(x => x.i), supportMid: sc.slice(q1, q2).map(x => x.i), neutral: sc.slice(q2, q3).map(x => x.i), opposeHigh: sc.slice(q3).map(x => x.i) }; }
      ...
      while (!PE.isComplete() && steps < 120) {
        steps++;
        const qq = PE.getNextQuestion() as { id: number; options?: string[] } | null;
        if (!qq) break;
        const q = PE.getQuestionDef(qq.id) as Q | null;
        if (!q) { PE.submitAnswer(qq.id, qq.options?.[0] ?? "unknown"); continue; }
        PE.submitAnswer(qq.id, decide(q, qq));
      }
      const r = PE.getResults() as { match: { id: string; name: string }; questionsAnswered: number };
      const rs = PE.getRespondentState() as { continuous: Record<string, { expectedPos: number; salience: number }> } | null;
    ...
    console.log(`  Archetype-${driveResult.matchId} MAT centroid: ${active.find(a => a.id === driveResult.matchId)?.nodes.MAT && active.find(a => a.id === driveResult.matchId)!.nodes.MAT!.kind === "continuous" ? (active.find(a => a.id === driveResult.matchId)!.nodes.MAT as { pos: number }).pos : "n/a"}`);
  ...
  const matValues = perPersona.map(p => p.matRespondent).filter(v => v !== null) as number[];
