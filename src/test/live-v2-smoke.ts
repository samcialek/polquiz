...
async function main() {
  ...
  while (steps < MAX_STEPS) {
    ...
    const qType = await page.evaluate(() => {
      if (document.querySelector('.bw-container')) return 'best_worst';
      if (document.querySelector('.allocation-container')) return 'allocation';
      if (document.querySelector('.ranking-container')) return 'ranking';
      if (document.querySelector('.tradeoff-container')) return 'tradeoff';
      if (document.querySelector('.slider-container')) return 'slider';
      if (document.querySelector('.mc-options')) return 'single_choice';
      return 'unknown';
    });
    ...
    if (qType === 'single_choice') {
      ...
      await page.evaluate(() => {
        const s = document.getElementById('main-slider') as HTMLInputElement | null;
        if (!s) return;
      ...
      const ok = await page.evaluate(() => {
        const sliders = Array.from(document.querySelectorAll('.allocation-slider')) as HTMLInputElement[];
        if (!sliders.length) return false;
        const each = Math.floor(100 / sliders.length);
        sliders.forEach((s, i) => { s.value = String(i === 0 ? 100 - each * (sliders.length - 1) : each); });
        const w = window as any;
        if (typeof w.updateAllocDisplay === 'function') w.updateAllocDisplay();
        return true;
      });
      if (!ok) {
