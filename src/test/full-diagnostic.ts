...
function scoreOptionEvidence(
  ...
  if (evidence.trbAnchor && archetype.trbAnchorPrior) {
    for (const [anchor, weight] of Object.entries(evidence.trbAnchor)) {
      const archWeight = archetype.trbAnchorPrior[anchor as keyof typeof archetype.trbAnchorPrior] ?? 0;
...
function scoreAllocationBucket(
  archetype: Archetype,
  bucket: AllocationBucketMap
): number {
  let score = 0;
  let count = 0;
 ...
 */
function softmaxSelect<T extends { score: number }>(
  items: T[],
  temp: number
): T {
  if (items.length === 0) throw new Error("softmaxSelect: empty items");
...
function generateNoisySingleChoice(
  archetype: Archetype,
  question: QuestionDef,
  state: RespondentState,
  temp: number
): { apply: () => void } | null {
  if (!question.optionEvidence) return null;
...
function generateNoisySlider(
  archetype: Archetype,
  question: QuestionDef,
  state: RespondentState,
  temp: number
): { apply: () => void } | null {
  if (!question.sliderMap) return null;
...
function generateNoisyAllocation(
  archetype: Archetype,
  question: QuestionDef,
  state: RespondentState,
  temp: number
): { apply: () => void } | null {
  if (!question.allocationMap) return null;
  ...
  if (allocTotal !== 100) {
    const maxKey = bucketNames.reduce((a, b) =>
...
function generateNoisyRanking(
  archetype: Archetype,
  question: QuestionDef,
  state: RespondentState,
  temp: number
): { apply: () => void } | null {
  if (!question.rankingMap) return null;
...
function generateNoisyPairwise(
  archetype: Archetype,
  question: QuestionDef,
  state: RespondentState,
  temp: number
): { apply: () => void } | null {
  if (!question.pairMaps) return null;
...
function generateNoisyBestWorst(
  archetype: Archetype,
  question: QuestionDef,
  state: RespondentState,
  temp: number
...
function generateRandomSingleChoice(
  question: QuestionDef,
  state: RespondentState
): { apply: () => void } | null {
  if (!question.optionEvidence) return null;
...
function generateRandomPairwise(
  question: QuestionDef,
  state: RespondentState
): { apply: () => void } | null {
  if (!question.pairMaps) return null;
...
interface ArchetypeSimResult {
  ...
  avgQuestions: number;
  avgMargin: number;
  avgTopPosterior: number;
  avgRank: number;
  confusionTargets: Record<string, number>;
  runDetails: ArchetypeRunResult[];
}
...
function simulateArchetypeRun(
  ...
  for (let i = 0; i < MAX_QUESTIONS; i++) {
    const available = questions.filter(
      (q) => !answeredIds.has(q.id) && q.touchProfile.length > 0
    );
...
function runArchetypeSimulations(
  archetypes: Archetype[],
  questions: QuestionDef[]
): ArchetypeSimResult[] {
  ...
  for (let ai = 0; ai < archetypes.length; ai++) {
    ...
    let totalRank = 0;
    const confusionTargets: Record<string, number> = {};
    ...
    const simResult: ArchetypeSimResult = {
    ...
    process.stdout.write(
      `\r${progressBar(globalRunCount, totalRuns)}  ` +
  ...
  for (const tier of tiers) {
    ...
    console.log(
      `  ${tier.padEnd(8)} top1=${(tierTop1 / tierRuns * 100).toFixed(1)}%  ` +
  ...
  for (const r of worstByAcc) {
    const confTop = Object.entries(r.confusionTargets)
      ...
      .join(", ");
    console.log(
      `  ${r.archetypeId} ${r.archetypeName.padEnd(35)} ` +
  ...
  const globalAttractors: Record<string, number> = {};
...
function runRandomWalks(
  ...
  const results: RandomWalkResult[] = [];
  ...
  for (const [id, cnt] of sortedAssign.slice(0, 20)) {
    ...
    console.log(
      `  ${id} ${(a?.name ?? "?").padEnd(35)} ${cnt} (${pct}%) [${ratio}x expected ${expectedPct}%]`
    );
  }
  console.log();
...
function writeResults(
  archetypeResults: ArchetypeSimResult[] | null,
  randomResults: RandomWalkResult[] | null,
  archetypes: Archetype[]
): void {
  ...
  if (archetypeResults) {
    ...
    fs.writeFileSync(
      path.join(outDir, "archetype-sims.json"),
      JSON.stringify({
        config: {
  ...
  if (randomResults) {
    ...
    fs.writeFileSync(
      path.join(outDir, "random-walks.json"),
      JSON.stringify({
        config: {
...
function main(): void {
  ...
  let archetypeResults: ArchetypeSimResult[] | null = null;
  let randomResults: RandomWalkResult[] | null = null;
