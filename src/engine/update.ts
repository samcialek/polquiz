...
function negativePartiesFromAnswers(optionKeys: readonly string[]): Set<string> | null {
...
function applyMetadataAnswer(
  state: RespondentState,
  q: QuestionDef,
  optionKeyOrKeys: string | readonly string[],
): void {
  ...
  for (const [bucket, weight] of Object.entries(allocation)) {
    ...
    if (map.categorical) {
      for (const [nodeId, catDist] of Object.entries(map.categorical)) {
        const node = state.categorical[nodeId as CategoricalNodeId];
        if (!node || !catDist) continue; // guard: undefined prototype reference
        const normFactor = NODE_NORM_FACTORS[nodeId] ?? 1;
...
export function applyPrioritySort(
  ...
  const perNodeTargetSum = new Map<string, number[]>();
  const perNodeWeightSum = new Map<string, number>();
  for (const item of allItems) {
    const map = q.rankingMap[item];
    if (!map?.continuous) continue;
    ...
    for (const [nodeId, evidence] of Object.entries(map.continuous)) {
      ...
      const itemTarget = raw.map((p: number) => p / rawSum);
  ...
  const skipPositionForNode = new Set<string>();
  for (const [nodeId, sumArr] of perNodeTargetSum) {
    ...
    let entropy = 0;
    for (const p of targetNorm) if (p > 0) entropy -= p * Math.log(p);
