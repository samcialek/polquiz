...
export function archetypeDistance(
  ...
  const useMoralCircle =
    !!state.moralCircle?.affinity && !!archetype.moralCircle;
  ...
  for (const nodeId of CONTINUOUS_NODES) {
    ...
    if ((useMoralCircle || useMorModule) && (MOR_MODULE_LEGACY as string[]).includes(nodeId)) continue;
