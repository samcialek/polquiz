...
import type { QuizQuestion, QuizProgress, QuizResults } from "./api.js";
import {
  initQuiz,
  getNextQuestion,
  submitAnswer,
...
// ---------------------------------------------------------------------------
// Question text — now sourced from q.promptFull on each question definition.
// The QUESTION_TEXT lookup table has been removed.
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// Standalone quiz renderer
// ---------------------------------------------------------------------------
 ...
 */
export function mountQuiz(container: HTMLElement, options: MountOptions = {}): void {
...
function renderProgress(progress: QuizProgress): HTMLElement {
...
function renderSingleChoice(q: QuizQuestion): HTMLElement {
...
function renderSlider(q: QuizQuestion): HTMLElement {
  const container = document.createElement("div");
...
function renderAllocation(q: QuizQuestion): HTMLElement {
  const container = document.createElement("div");
...
function renderRanking(q: QuizQuestion): HTMLElement {
  const container = document.createElement("div");
...
function renderBestWorst(q: QuizQuestion): HTMLElement {
  const container = document.createElement("div");
...
function renderPairwise(q: QuizQuestion): HTMLElement {
  const container = document.createElement("div");
 ...
 */
export function attachToExistingQuiz(options: AttachOptions): void {
  const { container, onComplete } = options;

  // Hide all pages
  container.querySelectorAll<HTMLElement>(".quiz-page").forEach(page => {
