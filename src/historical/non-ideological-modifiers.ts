...
export interface NonIdeologicalModifier {
  economic: number;     // in [-0.5, +0.5]
  incumbency: number;   // in [0, +0.3]
  charisma: number;     // in [-0.5, +0.5]
  total: number;        // weighted and capped, in [-0.3, +0.3]
}

interface ElectionEntry {
  incumbent_party: string | null;
  raw_econ: number;
  tier: "quarterly_1948_plus" | "annual_1929_1947" | "binary_pre_1929";
  tier_category?: string;
  gdp_growth_pct?: number;
  gdp_q2_annualized?: number;
...
interface DataFile {
  ...
  elections: Record<string, ElectionEntry>;
...
function loadAndValidate(): DataFile {
  const parsed = dataJson as unknown as DataFile;
...
const DATA: DataFile = loadAndValidate();
 ...
 */
export function historicalToCanonical(name: string, year: number): string {
...
function economicComponent(rawEcon: number, candidateParty: string, incumbentParty: string | null): number {
 ...
 * contribute no nudge.)
 */
export function getNonIdeologicalModifier(
  year: number,
  candidateKey: string,
