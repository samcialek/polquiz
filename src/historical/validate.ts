...
function getCatPeak(arch: Archetype, node: string): number {
  ...
  const probs = (tmpl as CategoricalTemplate).probs;
  return probs.indexOf(Math.max(...probs));
}

function getVote(results: VoteResult[], archId: string, year: number): string {
  const r = results.find((r) => r.archetypeId === archId);
  return r ? r.votes[year] : "UNKNOWN";
}

function getVoteByName(results: VoteResult[], name: string, year: number): string {
...
function isLaborArchetype(arch: Archetype): boolean {
...
function isEvangelicalArchetype(arch: Archetype): boolean {
...
function isSuburbanEducatedArchetype(arch: Archetype): boolean {
...
function isBlackCoalitionArchetype(arch: Archetype): boolean {
...
function isSocialConservativeArchetype(arch: Archetype): boolean {
  const morPos = getPos(arch, "MOR");
  const cdPos = getPos(arch, "CD");
...
function isFreeMarketArchetype(arch: Archetype): boolean {
  const matPos = getPos(arch, "MAT");
  const matSal = getSal(arch, "MAT");
...
function isPopulistArchetype(arch: Archetype): boolean {
...
function runValidation(results: VoteResult[]): ValidationCheck[] {
  ...
  {
    const spotChecks: Array<{ archName: string; year: number; expected: string }> = [
...
function printReport(checks: ValidationCheck[]): void {
  console.log("\n═══════════════════════════════════════════════════════════════");
...
function printCoalitionBreakdown(results: VoteResult[]): void {
  console.log("\n═══════════════════════════════════════════════════════════════");
  console.log("  COALITION BREAKDOWN BY ELECTION");
  console.log("═══════════════════════════════════════════════════════════════\n");

  const coalitions: Array<{ name: string; filter: (a: Archetype) => boolean }> = [
...
function writeCSVs(results: VoteResult[]) {
  const fs = require("fs");
