...
function computeWeightedDistance(
  ...
  for (const node of CONTINUOUS_NODES) {
    if (node === "ENG") continue; // ENG is not a scoring dimension
 ...
 */
export function computeRegimeAlignment(
  arch: Archetype,
  candidates: CandidateProfile[],
  ctx: ElectionContext,
): { alignment: number; bestCandidate: string; bestScore: number } {
  let bestAlignment = -3;
  let bestCandidate = "";
 ...
 */
export function alignmentToTurnout(alignment: number, intensity: number = 1.0): number {
 ...
 */
export function computeTurnoutFromAlignment(
  arch: Archetype,
  candidates: CandidateProfile[],
  ctx: ElectionContext,
): { alignment: number; turnoutProbability: number; votes: boolean } {
 ...
 */
export function gaussianVoteChoice(
  arch: Archetype,
  candidates: CandidateProfile[],
  ctx: ElectionContext,
): { bestCandidate: string; bestAlignment: number; scores: Record<string, number> } {
