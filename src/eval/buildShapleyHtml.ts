...
interface ShapRow { posMean: number; salMean: number; arches: number; }
interface CatRow  { catL1Mean: number; salMean: number; arches: number; }
interface ShapData {
  K: number;
  numArchetypes: number;
  elapsedSeconds: number;
  global: {
    continuous: Record<string, Record<string, ShapRow>>;
    categorical: Record<string, Record<string, CatRow>>;
  };
}

const raw = JSON.parse(fs.readFileSync(IN_JSON, "utf8")) as ShapData;
...
const CATEGORICAL_SET = new Set(CATEGORICAL_NODES as unknown as string[]);

interface Cell {
  qid: number;
  node: string;
  primary: number;   // posMean (cont) | catL1Mean (cat)
  salience: number;  // salMean
  arches: number;
}

const qids = REPRESENTATIVE_QUESTIONS.map(q => q.id).sort((a, b) => a - b);
const qById = new Map(REPRESENTATIVE_QUESTIONS.map(q => [q.id, q] as const));

const cells: Record<number, Record<string, Cell>> = {};
for (const qid of qids) cells[qid] = {};
...
// Per-column max
const colMaxPrimary: Record<string, number> = {};
const colMaxSalience: Record<string, number> = {};
for (const n of NODE_ORDER) {
  let mp = 0, ms = 0;
  for (const qid of qids) {
    const c = cells[qid]?.[n];
    if (!c) continue;
    if (c.primary > mp) mp = c.primary;
    if (c.salience > ms) ms = c.salience;
  }
...
// Per-column top-3
function topNSet(field: "primary" | "salience", n: string, k = 3): Set<number> {
...
const topPosByNode: Record<string, Set<number>> = {};
const topSalByNode: Record<string, Set<number>> = {};
for (const n of NODE_ORDER) {
  topPosByNode[n] = topNSet("primary", n);
  topSalByNode[n] = topNSet("salience", n);
}

function rowAgg(field: "primary" | "salience"): Record<number, number> {
...
// ---------------------------------------------------------------------------
function optionSummary(q: QuestionDef): string {
  const lines: string[] = [];
  const labelFor = (key: string) => q.optionLabels?.[key] ?? key;
  ...
  if (q.optionEvidence) {
    ...
    if (keys.length > 8) lines.push(`  • … +${keys.length - 8} more options`);
  } else if (q.sliderMap) {
    ...
    for (const k of keys.slice(0, 8)) lines.push(`  • ${k}${q.optionLabels?.[k] ? ` — ${q.optionLabels[k]}` : ""}`);
    if (keys.length > 8) lines.push(`  • … +${keys.length - 8} more items`);
...
function fmt(v: number, p = 3): string { return v.toFixed(p); }
...
// ---------------------------------------------------------------------------
// Data block embedded in HTML for rich per-question tooltip via JS
// ---------------------------------------------------------------------------
interface QInfo {
  qid: number;
  promptShort: string;
...
const qInfo: Record<number, QInfo> = {};
...
function renderHeatmap(
  title: string,
  field: "primary" | "salience",
  colMax: Record<string, number>,
  topByNode: Record<string, Set<number>>,
  rowAggMap: Record<number, number>,
  heatColor: (node: string) => string,
  cellTipMetricLabel: (isCat: boolean) => string,
): string {
  ...
  for (const qid of sorted) {
    ...
    for (const n of NODE_ORDER) {
      const c = cells[qid]?.[n];
      if (!c) { cellsHtml.push(`<td class="cell empty"></td>`); continue; }
      const val = c[field] ?? 0;
  ...
  const clusterHeader: string[] = [];
  let prev = ""; let span = 0;
  for (const n of NODE_ORDER) {
    const c = CLUSTER[n]!;
    if (c === prev) { span++; continue; }
    if (span) clusterHeader.push(`<th colspan="${span}" class="cluster" style="background: ${CLUSTER_COLOR[prev]}22; color: ${CLUSTER_COLOR[prev]};">${prev}</th>`);
    prev = c; span = 1;
  }
  if (span) clusterHeader.push(`<th colspan="${span}" class="cluster" style="background: ${CLUSTER_COLOR[prev]}22; color: ${CLUSTER_COLOR[prev]};">${prev}</th>`);
...
// ---------------------------------------------------------------------------
function summaryLines(field: "primary" | "salience"): string {
  const lines: string[] = [];
  for (const n of NODE_ORDER) {
...
const posHeatmap = renderHeatmap(
  "Position Shapley (continuous: E[pos] shift · categorical: catDist L1 shift)",
  "primary",
  colMaxPrimary,
  topPosByNode,
  rowAggPrimary,
  (n) => CATEGORICAL_SET.has(n) ? "#c77dbb" : "#2fb3a5",
  (isCat) => isCat ? "catL1" : "posΔ",
);

const salHeatmap = renderHeatmap(
  "Salience Shapley (E[sal] shift)",
  "salience",
  colMaxSalience,
  topSalByNode,
  rowAggSalience,
  () => "#f0a050",   // orange for salience
  () => "salΔ",
);
