...
<div class="container">
  ...
  <div class="caveats">
    ...
    <p style="margin: 6px 0;"><strong>Plurality match 4/5</strong> (2008 / 2016 / 2020 / 2024 correct; 2012 still wrong — the only cycle where Obama's strong centrist positioning makes the model's D-prediction beat its Abstain prediction). Avg 4-way TV distance <strong>5.96pp</strong>. Improved from 2/5 / 9.20pp via two calibration sweeps on the proper 4-way TV target — Round-2 (loyalty 1.0 → 3.0, salPower 1.5 → 1.3) and Round-3 (loyalty 3.0 → 5.0, salPower 1.3 → 1.1). Loyalty hit the upper grid edge in both rounds with diminishing returns; we treat 5.0 as the calibration ceiling absent structural moves.</p>
    ...
    <ul>
      ...
      <li><strong>2012:</strong> avg gap 13.7pp (was 18.1pp). CD via CC324/CC326/CC332J, CU via CC322_1..6 immigration battery, MAT via CC332* roll-call battery + CC328/CC329 forced-choice, ZS via CC325/CC332F/CC302.</li>
      <li><strong>2016 / 2020 / 2024:</strong> abstention now 36.9–38.6% vs 34.6–42.0% actual (within 5pp on all cycles). Remaining D/R gaps in the 6–11pp range from three additive biases at the vote-prediction layer:
        <ul style="margin-top: 4px;">
          ...
          <li><strong>partyID IS now passed</strong> via pid7 → PartyID translation; the partisan-loyalty multiplier is on. CCES pid7 distribution skews D in modern cycles, so partyID amplifies the survey's D-skew somewhat — which is an honest reflection of the survey weights, not a bug. Net 2020/2024 gap improved 3pp / 1.6pp from this lever; 2016 gap shifted +1pp (CCES16 pid7 pool more D-tilted than the 2016 vote split).</li>
          <li><strong>moralBoundaries — Phase E compatibility pass landed.</strong> 2008 now has real_signal on religious (V215/V216/V217), political_camp (CC307a), ethnic_racial (CC313 affirmative-action 4-pt direction-agnostic), and gender (CC316f SSM-ban direction-agnostic). 2012 has real_signal on gender (CC326 + CC332J direction-agnostic) on top of the existing pid7-derived political_camp and pew_*-derived religious. Each source documented inline as future moralCircleAffinity prior evidence per ADR-007.</li>
      <li><strong>ADR-007 Stage F (moralCircleDistance) cutover landed.</strong> The runner builds a per-respondent <code>MoralCircleAffinity</code> from the mapper's existing boundary salience + legacy MOR position (universalAffinity ← MOR pos 1..5 → 0..100; scopedAffinities.ideological ← max(mapper.ideological, mapper.political_camp); other scopes ← mapper boundary salience). Threaded through <code>predictVote</code> as the 11th parameter; <code>ideologicalDistance</code> now prefers <code>moralCircleDistance(state, candidate.moralCircle)</code> over the legacy <code>morTargetVectorDistance</code> when both sides have moralCircle data — exactly mirroring the T7 archetypeDistance gate pattern. Geometry: 0.40 universal-affinity scalar diff + 0.40 excess-vector L2 + 0.20 active-boundary Jaccard miss. Closes 0.6–1.4pp per cycle vs the pre-Stage-F baseline.</li>
      <li><strong>respondentVoteChoice constant refit landed.</strong> An 18-config calibration sweep (audit at <code>respondent-vote-choice-calibration.md/.json</code>) tuned <code>SALIENCE_POWER</code> 2.0 → 1.5 and <code>PARTY_LOYALTY_BASE</code> 0.40 → 1.00. The party-loyalty bump was the dominant lever — at 0.40, cross-pressured partisans drifted to whichever candidate was issue-closer (which on CCES-weighted samples systematically favored D because population-center policy is closer to D candidates than to recent R). At 1.00, PF=5 partisans face a 2.0× distance multiplier to out-of-party candidates (was 1.4×), routing them back to their party. Combined with the SALIENCE_POWER drop, the avg D/R-of-voters gap closed from 9.79pp → 8.22pp across all 5 cycles.</li>
        </ul>
      </li>
