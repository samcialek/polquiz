...
interface CandidateEntry {
  ...
  archId: string;
  tier: string;
  granularity: "3-token" | "2-token" | "1-token";
}
 ...
 *   mid-position nodes.
 */
function selectArchetypeMergerTokens(entries: TokenEntry[]): TokenEntry[] {
...
for (const arch of ARCHETYPES) {
  ...
  if (picked.length === 3) {
    for (const [a, b] of [[0, 1], [0, 2], [1, 2]] as const) {
      const subset: TokenEntry[] = [picked[a]!, picked[b]!];
      candidates.push({
...
if (collisions.length > 0) {
  ...
  for (const c of collisions.slice(0, 20)) {
    ...
    for (const r of c.runners.slice(0, 4)) console.log(`    · ${r}`);
  }
}
