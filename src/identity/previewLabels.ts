...
import {
  ...
  labelForRespondent,
  tokenizeArchetype,
  type LabelerRespondentState,
  type MergerTable,
} from "./archetypeLabeler.js";

const mergerTable: MergerTable = JSON.parse(readFileSync("src/identity/mergerTable.json", "utf8"));

// ──────────────────────────────────────────────────────────────────────────────
// Archetype centroid pass
// ──────────────────────────────────────────────────────────────────────────────
...
for (const arch of ARCHETYPES) {
  ...
  if (r.source !== "lexicon") mergerHitCount++;
  archRows.push({
    id: arch.id,
    oldName: arch.name,
    newLabel: r.label,
...
// ──────────────────────────────────────────────────────────────────────────────
// Real respondent dumps (Sam's two recent runs)
// ──────────────────────────────────────────────────────────────────────────────

interface PersonaCase {
  label: string;
  matchedArch: string;
  state: LabelerRespondentState;
}

const samRun1: PersonaCase = {
...
for (const p of personaCases) {
  ...
  if (salientList) lines.push(`- Salient continuous nodes (≥2.5): ${salientList}`);
  lines.push("");
}
