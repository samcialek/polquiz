...
import {
  ...
  EPS_LEXICON,
  AES_LEXICON,
  MORAL_CIRCLE_SCOPE_LEXICON,
  DEFAULT_MERGER_TABLE,
  COMPRESSION_TABLE,
  composeLabel,
  signatureOf,
  type TokenEntry,
} from "./archetypeLabeler.js";

const EMPTY_MERGER_TABLE = {};

interface TokenSlot {
  slot: string;        // "MAT", "EPS", "AES", "MORAL_CIRCLE"
  bin: string;         // "low", "mid", "high", "Statesman", "national", etc.
  token: string;
  isMid: boolean;      // mid bin tokens only appear at rank-1
...
function toEntry(t: TokenSlot, isTop1: boolean): TokenEntry {
...
//   1-token: every token (mids included — they're top-1 by definition).
//   2-token: top-1 is any token; top-2 is any non-mid token from a different slot.
//   3-token: top-1 any; top-2 + top-3 non-mid, all different slots.
//
// We dedupe by signature so the CSV has one row per unique outcome.
// ──────────────────────────────────────────────────────────────────────────────
...
function add(entries: TokenEntry[]): void {
...
for (let i = 0; i < allTokens.length; i++) {
  ...
  for (let j = 0; j < allTokens.length; j++) {
    ...
    if (top2.slot === top1.slot) continue; // no two tokens from same slot
    if (top2.isMid) continue;              // mid only at rank-1
...
for (let i = 0; i < allTokens.length; i++) {
  ...
  for (let j = 0; j < allTokens.length; j++) {
    if (i === j) continue;
    const top2 = allTokens[j]!;
    if (top2.slot === top1.slot) continue;
    if (top2.isMid) continue;
    for (let k = j + 1; k < allTokens.length; k++) {
      const top3 = allTokens[k]!;
      if (top3.slot === top1.slot || top3.slot === top2.slot) continue;
      if (top3.isMid) continue;
...
const csv: string[] = ["token_count,source,compressed,signature,label,raw_lexicon,tokens"];
