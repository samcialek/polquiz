...
|------|-----------|------|-------|-----------|
| PF | Political Fusion | continuous | 1-5 | activation-only |
| TRB | Tribe | continuous | 1-5 | activation-only |
| ENG | Engagement Level | continuous | 1-5 | activation-only |

## Position vs. Salience — Two Independent Dimensions (Except SELF)
Every person has both a **position** and a **salience** on each ENDS/MEANS/REALITY node:
...
**Exception — SELF-cluster nodes (PF, TRB, ENG) are activation-only.** Per ADR-005, position and salience collapse into a single dimension for these three nodes. Reason: they describe the respondent's *relationship to* politics, not a position *about* politics. You can hold an opinion that doesn't matter to you; you can't have a relationship that doesn't matter to you. "Strongly non-partisan but highly salient" is structurally incoherent. The `pos` (1-5) scale doubles as the activation scale: 1 = tuned out / non-partisan / non-tribal; 5 = all in / partisan-fused / tribal. The `sal` field is absent from SELF-node archetype entries; the engine skips `salDist` reads/writes for these nodes (see `src/config/nodes.ts:SELF_NODES` and `isSelfNode`).
...
- `role: "salience"` → the response tells us how much the person *cares about* that node (non-SELF nodes only; SELF-node salience touches were stripped in the ADR-005 refactor)

At the **archetype level**, each non-SELF node is defined by position (pos 1-5) and salience (sal 0-3), plus optional anti-positions. SELF nodes carry only `pos` and optional `anti`. Categorical nodes use probability distributions over their 6 categories plus salience. The engine matches a respondent to archetypes using all these dimensions.

## Key Numbers — DO NOT CHANGE
- **121 active archetypes** (+3 deactivated kept in array for ID stability: 019, 023, 025 → 124 total entries). 115 base archetypes + 6 identity-primary archetypes (IDs 141-146). Priors are uniform at 1/121. History: 132 → 124 → 122 → 115 → 118 → 121 (final, post identity-primary integration). The `prior` field was removed from `Archetype` in Phase 3 and stays removed post-ADR-003 rollback — Diagnostic 1 established priors were always uniform and did not contribute to ranking.
...
- **ADR-005 — SELF-cluster activation-only collapse.** In effect (2026-04-23). PF, TRB, ENG lose their separate `salDist` axis; `pos` (1-5) becomes the single activation scale. Archetype entries strip the `sal` field for these three nodes. Engine `update.ts` / `archetypeDistance.ts` gated on `isSelfNode(nodeId)`. Post-refactor regression: top-1 116/121 = 95.9% at σ=0 (vs 96.7% pre-refactor, -0.8pp within tolerance). See `results/architecture/ADR-005-self-cluster-collapse.md`.
