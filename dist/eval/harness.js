...
function generateSingleChoice(a, q, s) {
    if (!q.optionEvidence)
        return null;
...
function generateAllocation(a, q, s) {
    if (!q.allocationMap)
        return null;
...
function generateRanking(a, q, s) {
    if (!q.rankingMap)
        return null;
...
function generatePairwise(a, q, s) {
    if (!q.pairMaps)
        return null;
    const answers = {};
...
function generateBestWorst(a, q, s) {
    if (!q.bestWorstMap)
        return generateRanking(a, q, s);
...
function generateDualAxis(a, q, s) {
    if (!q.dualAxisMap)
        return null;
...
function generateConjoint(a, q, s) {
    return generateSingleChoice(a, q, s);
}
function generateAnswer(a, q, s) {
    switch (q.uiType) {
...
// ---------------------------------------------------------------------------
function determineStopFired(state, archetypes, questionsAnswered) {
    if (questionsAnswered >= 55)
        return "hardCap";
    ...
    const fired = [];
    if (questionsAnswered >= 20 && dLeader <= 6.0 && gapRatio >= 0.25 && cc >= 8)
        fired.push("ultraConf");
    if (questionsAnswered >= 45 && dLeader <= 12.0 && gapRatio >= 0.03 && cc >= 4)
        fired.push("lateGame");
    if (questionsAnswered >= 35 && dLeader <= 10.0 && gapRatio >= 0.05 && cc >= 6)
        fired.push("secondary");
    if (questionsAnswered >= 25 && dLeader <= 8.0 && gapRatio >= 0.10 && cc >= 3)
        fired.push("primary");
...
function diffSnapshots(before, after) {
    ...
    for (const nodeId of CONTINUOUS_NODES) {
        ...
        if (posChanged || salChanged) {
            const d = { nodeId, kind: "continuous" };
            if (posChanged) {
                d.posBefore = round6(bf.posDist);
                d.posAfter = round6(af.posDist);
            }
            if (salChanged) {
                d.salBefore = round6(bf.salDist);
                d.salAfter = round6(af.salDist);
            }
            deltas.push(d);
        }
    }
    for (const nodeId of CATEGORICAL_NODES) {
        ...
        if (catChanged || salChanged) {
            const d = { nodeId, kind: "categorical" };
            if (catChanged) {
                d.catBefore = round6(bf.catDist);
                d.catAfter = round6(af.catDist);
            }
            if (salChanged) {
                d.salBefore = round6(bf.salDist);
                d.salAfter = round6(af.salDist);
...
let _familyIndexCache = null;
function getFamilyIndex(archetypes) {
...
export function simulateOne(target, archetypes, questions, opts = {}) {
    ...
    for (let i = 0; i < maxQuestions; i++) {
        const available = questions.filter(q => !answeredIds.has(q.id) && q.touchProfile.length > 0 && isQuestionEligible(state, q));
        ...
        if (answer) {
            ...
            if (questionsAnswered >= 25 && shouldStop(state, activeArchetypes))
                break;
        }
        else {
            answeredIds.add(nextQ.id);
