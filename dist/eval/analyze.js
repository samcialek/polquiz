...
// Archetype metadata
// ---------------------------------------------------------------------------
const ARCH_META = Object.fromEntries(ARCHETYPES.map(a => [a.id, { id: a.id, name: a.name, tier: a.tier, prior: a.prior }]));
const ACTIVE_IDS = ARCHETYPES.filter(a => !DEACTIVATED.has(a.id)).map(a => a.id);
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function entropyOfPosterior(p) {
    let h = 0;
...
function credibleSetSize(p, alpha) {
...
function makeSigmaBlock(sigma, file) {
    ...
    for (const a of ARCHETYPES)
        perArch[a.id] = makePerArchStats(a.id);
    const preregStats = {};
...
async function processSigmaFile(sigma, file) {
    ...
    for await (const line of rl) {
        ...
        if (r.correct)
            block.stopByCorrect[r.stopFired].correct++;
        // Family pair totals
        if (r.familyPairDetected) {
            block.familyPairTotals.detected++;
            const inPair = r.familyPairDetected.top1 === r.archetypeId || r.familyPairDetected.top2 === r.archetypeId;
            if (inPair)
                block.familyPairTotals.correctWhenDetected++;
        }
        ...
        for (const [a, b] of PREREG_FAMILIES) {
            if (r.archetypeId === a || r.archetypeId === b) {
                ...
                if (top2Ids.includes(partner))
                    stats.partnerAppearedInTop2++;
                if (top5Ids.includes(partner))
                    stats.partnerAppearedInTop5++;
                if (r.familyPairDetected &&
...
// CSV writers
// ---------------------------------------------------------------------------
function writeConfusionCsv(block, outPath) {
...
function writePerArchCsv(blocks, outPath) {
    ...
    for (const a of ARCHETYPES) {
        ...
        for (const block of blocks) {
            ...
            const familyCorrectPct = p.familyDetectedCount
                ...
                : 0;
            row.push(p.runs, top1Pct.toFixed(2), top3Pct.toFixed(2), top5Pct.toFixed(2), entropy.toFixed(4), credset.toFixed(2), meanQ.toFixed(2), medianQ, familyPct.toFixed(2), familyCorrectPct.toFixed(2));
        }
        lines.push(row.join(","));
    }
    fs.writeFileSync(outPath, lines.join("\n"));
}
function writeTierConfusionCsv(block, outPath) {
    const tiers = ["T1", "T2", "MEANS", "GATE", "REALITY"];
    const tierOf = (id) => ARCH_META[id]?.tier ?? "?";
    const matrix = {};
...
function writeStopRuleCsv(blocks, outPath) {
    const sigmas = blocks.map(b => b.sigma);
    const keys = new Set();
    for (const b of blocks)
        for (const k of Object.keys(b.stopTotals))
            keys.add(k);
...
function writeConfusionPairsCsv(block, outPath, limit = 20) {
    const pairs = [];
    for (const [truth, preds] of Object.entries(block.confusion)) {
        if (DEACTIVATED.has(truth))
            continue;
...
function computeFlaggedLists(blocks) {
    const [s0, s05, s15] = blocks;
    if (!s0 || !s05 || !s15)
        throw new Error("Expected 3 sigma blocks");
    ...
    for (const a of ARCHETYPES) {
        if (DEACTIVATED.has(a.id))
            continue;
        const p = s0.perArch[a.id];
        const pct = p.runs ? (p.top1 / p.runs) * 100 : 0;
        if (pct < 50)
            nonIdentifiable.push({ id: a.id, name: a.name, sigma0_top1_pct: pct });
    }
    ...
    for (const a of ARCHETYPES)
        attractorCounts[a.id] = { s0: 0, s05: 0, s15: 0 };
    const addAttract = (block, field) => {
        for (const [truth, preds] of Object.entries(block.confusion)) {
            if (DEACTIVATED.has(truth))
                continue;
            for (const [pred, count] of Object.entries(preds)) {
    ...
    attractors.sort((a, b) => (b.missesAttracted_sigma0 + b.missesAttracted_sigma05 + b.missesAttracted_sigma15) -
    ...
    const familyPairUnderCalibrated = [];
    for (const block of blocks) {
        for (const a of ARCHETYPES) {
            if (DEACTIVATED.has(a.id))
                continue;
...
function computeFamilyCalibration(blocks) {
    return blocks.map(b => {
...
function writeReport(blocks, flagged, familyCal, driftFlags, manifest, outPath) {
    const [s0, s05, s15] = blocks;
    if (!s0 || !s05 || !s15)
        throw new Error("Expected 3 sigma blocks");
    const agg = (b) => {
    ...
    for (const f of flagged.fragile.slice(0, 25)) {
        lines.push(`| ${f.id} | ${f.name.padEnd(39)} | ${fmt(f.sigma0_top1, 1).padStart(7)}%  | ${fmt(f.sigma05_top1, 1).padStart(9)}%  | ${fmt(f.drop, 1).padStart(4)}pp |`);
    }
    if (!flagged.fragile.length)
        lines.push("| _(none)_ | | | | |");
    lines.push("");
    ...
    for (const a of flagged.attractors) {
        lines.push(`| ${a.id} | ${a.name.padEnd(39)} | ${String(a.missesAttracted_sigma0).padStart(4)} | ${String(a.missesAttracted_sigma05).padStart(5)} | ${String(a.missesAttracted_sigma15).padStart(5)} |`);
    }
    lines.push("");
    ...
    for (const d of flagged.diffuse.slice(0, 15)) {
        lines.push(`| ${d.id} | ${d.name.padEnd(39)} | ${fmt(d.meanCredset_sigma0, 1).padStart(12)} | ${fmt(d.meanEntropy_sigma0, 3).padStart(12)} |`);
    }
    lines.push("");
    ...
    for (const f of familyCal) {
        lines.push(`| ${f.sigma} | ${String(f.detected).padStart(8)} | ${fmt(f.detectedPct, 1).padStart(8)}%  | ${fmt(f.correctWhenDetectedPct, 2).padStart(20)}%  | ${f.pass ? "PASS" : "FAIL"}        |`);
    }
    lines.push("");
    ...
    for (const u of flagged.familyPairUnderCalibrated.slice(0, 25)) {
        lines.push(`| ${u.id} | ${u.name.padEnd(29)} | ${u.sigma} | ${fmt(u.correctWhenDetected_pct, 1).padStart(20)}%  | ${String(u.detectedRuns).padStart(3)} |`);
    }
    if (!flagged.familyPairUnderCalibrated.length)
        lines.push("| _(none)_ | | | | |");
    lines.push("");
    ...
    for (const block of blocks) {
        ...
        for (const [a, b] of PREREG_FAMILIES) {
            ...
            lines.push(`| ${key.padEnd(9)} | ${String(s.runs).padStart(4)} | ${fmt(top2Pct, 1).padStart(12)}%    | ${fmt(top5Pct, 1).padStart(12)}%    | ${fmt(famPct, 1).padStart(18)}%     |`);
        }
        lines.push("");
    }
    ...
    for (const block of blocks) {
        ...
        for (const k of sortedKeys) {
            ...
            lines.push(`| ${k.padEnd(22)} | ${String(count).padStart(6)} | ${fmt(pct, 2).padStart(5)}% | ${fmt(corrPct, 2).padStart(7)}% |`);
        }
        lines.push("");
    }
...
async function main() {
    ...
    fs.writeFileSync(path.join(OUT_DIR, "flagged-lists.json"), JSON.stringify(flagged, null, 2));
    fs.writeFileSync(path.join(OUT_DIR, "family-pair-calibration.json"), JSON.stringify(familyCal, null, 2));
    fs.writeFileSync(path.join(OUT_DIR, "preregistered-families.json"), JSON.stringify(blocks.map(b => ({ sigma: b.sigma, stats: b.preregStats })), null, 2));
    fs.writeFileSync(path.join(OUT_DIR, "drift-flags.json"), JSON.stringify(driftFlags, null, 2));
