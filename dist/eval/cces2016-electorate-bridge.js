...
const CODEBOOK_LABEL_OVERRIDES = {
...
let ACTIVE_EVIDENCE_BLOCK_SCALES = {};
let ACTIVE_EVIDENCE_JITTER = { amplitude: 0, seed: "baseline" };
function setEvidenceStress(blockScales = {}, jitterAmplitude = 0, jitterSeed = "baseline") {
    ACTIVE_EVIDENCE_BLOCK_SCALES = blockScales;
    ACTIVE_EVIDENCE_JITTER = { amplitude: jitterAmplitude, seed: jitterSeed };
}
function evidenceBlock(variable) {
    if (variable === "ideo5")
        return "ideology";
    if (variable === "pid3" || variable === "pid7" || variable.startsWith("CC16_421"))
        return "party_id";
    if (variable === "CC16_328")
        return "primary_vote";
    if (variable.startsWith("CC16_422"))
        return "racial_attitudes";
    if (variable.startsWith("CC16_320")
        || variable === "CC16_302"
        || variable === "CC16_303"
        || variable === "CC16_304"
        || variable === "CC16_307"
    ...
    if (variable.startsWith("CC16_301")
        ...
        || variable === "CC16_415r")
        return "policy";
    if (variable === "newsint" || variable === "votereg" || variable === "CC16_327" || variable.startsWith("CC16_417"))
        return "engagement";
    if (variable === "race" || variable === "hispanic" || variable === "Hispanic_origin_5" || variable === "sexuality" || variable === "trans" || variable === "gender")
        return "demographics";
    if (variable.startsWith("pew_") || variable === "union" || variable === "unionhh")
        return "religion_union";
    return "other";
}
function scaledEvidenceWeight(variable, target, role, baseWeight) {
...
function asNum(value) {
    if (value == null || value === "" || value === "." || value === "NA")
        return null;
    const n = Number(value);
    return Number.isFinite(n) ? n : null;
}
function validCode(value, min = 1, max = 7) {
...
function fivePoint(value) {
    return validCode(value, 1, 5) ? value : null;
}
function invertFive(value) {
    return validCode(value, 1, 5) ? 6 - value : null;
}
function fourApprovalToPos(value, approveHigh = true) {
    if (!validCode(value, 1, 4))
        return null;
    const pos = 1 + ((value - 1) / 3) * 4;
    return approveHigh ? 6 - pos : pos;
}
function issueImportanceToSal(value) {
    if (!validCode(value, 1, 5))
        return null;
    return clamp(3.2 - (value - 1) * 0.75, 0, 3);
}
function binaryPolicyPos(value, supportPos, opposePos) {
    if (value === 1)
        return supportPos;
    if (value === 2)
        return opposePos;
...
function binarySal(value, sal = 2.0) {
    return value === 1 || value === 2 ? sal : null;
}
function codebookLabels() {
    ...
    for (const [k, v] of Object.entries(CODEBOOK_LABEL_OVERRIDES))
        labels.set(k, v);
    return labels;
}
...
class SignatureBuilder {
    ...
    anchorTouches = 0;
    constructor() {
        this.continuous = Object.fromEntries(CONTINUOUS_NODES.map((node) => [node, {
        ...
        this.anchorMass = Object.fromEntries(TRB_ANCHOR_ORDER.map((a) => [a, 0.08]));
    }
    addContinuous(node, pos, sal, weight) {
        if (pos == null && sal == null)
            return;
    ...
    addCategory(node, dist, sal, weight) {
        if (!dist && sal == null)
            return;
        ...
        if (dist) {
            ...
            if (total > 0) {
                for (let i = 0; i < 6; i++)
                    p.dist[i] += (Math.max(0, dist[i] ?? 0) / total) * weight;
                p.weight += weight;
            }
        }
        if (sal != null) {
    ...
    finalize() {
        ...
        const anchorDist = TRB_ANCHOR_ORDER.map((a) => this.anchorMass[a] / anchorTotal);
...
function priorSal(node) {
    if (node === "ENG" || node === "PF" || node === "TRB")
        return 1.2;
    return 1.0;
}
function priorWeight(node) {
    if (node === "ENG" || node === "PF" || node === "TRB")
        return 1.0;
    return 0.8;
}
function posteriorSd(totalWeight, touches) {
    return clamp(1.25 / Math.sqrt(totalWeight + touches * 0.7), 0.22, 1.15);
}
function isSelfNodeId(node) {
    return node === "PF" || node === "TRB" || node === "ENG";
}
function continuousSpec(variable, label, node, role, mapping, weight, pos, sal, notes = "") {
    const block = evidenceBlock(variable);
    return {
...
function anchorSpec(variable, label, anchor, mapping, weight, active, notes = "") {
    const block = evidenceBlock(variable);
    const target = `TRB_ANCHOR:${anchor}`;
    return {
        variable, label, block, target, role: "anchor", mapping, weight, notes,
        apply: (row, builder) => {
            if (active(row.num(variable), row))
                builder.addAnchor(anchor, scaledEvidenceWeight(variable, target, "anchor", weight));
        },
    };
}
function categorySpec(variable, label, node, mapping, weight, dist, sal, notes = "") {
    const block = evidenceBlock(variable);
    return {
...
function buildCrosswalk(labels) {
    const L = (v) => labels.get(v) ?? CODEBOOK_LABEL_OVERRIDES[v] ?? v;
    const specs = [];
    specs.push(continuousSpec("ideo5", L("ideo5"), "MAT", "position", "1 very liberal -> MAT 1; 5 very conservative -> MAT 5", 1.15, v => fivePoint(v), () => 1.8, "Broad ideology is used as a weak prior, not a vote label."), continuousSpec("ideo5", L("ideo5"), "CD", "position", "1 very liberal/open -> CD 1; 5 conservative/closed -> CD 5", 1.0, v => fivePoint(v), () => 1.6), continuousSpec("ideo5", L("ideo5"), "CU", "position", "1 very liberal/pluralist -> CU 5; 5 conservative/particularist -> CU 1", 0.9, v => invertFive(v), () => 1.4), 
    ...
    continuousSpec("ideo5", L("ideo5"), "MOR", "position", "1 very liberal -> wide moral scope (MOR 5); 5 conservative -> narrow scope (MOR 1)", 0.8, v => invertFive(v), () => 1.3), categorySpec("ideo5", L("ideo5"), "EPS", "ideology certainty: extremes -> autonomous/intuitionist; moderates -> institutionalist/empiricist", 0.55, ideologyToEps, v => validCode(v, 1, 5) ? 1.0 : null));
    ...
    specs.push(continuousSpec("CC16_320a", L("CC16_320a"), "PF", "activation", "Obama approval/disapproval weakly confirms Democratic/Republican fusion", 0.35, (v, row) => partyFusionFromObamaApproval(v, row), () => null), continuousSpec("CC16_320c", L("CC16_320c"), "PRO", "position", "approve Supreme Court -> procedural deference", 0.55, v => fourApprovalToPos(v), v => validCode(v, 1, 4) ? 1.2 : null));
    specs.push(continuousSpec("CC16_302", L("CC16_302"), "ONT_S", "position", "national economy better -> system trust; worse -> system distrust", 0.65, v => validCode(v, 1, 5) ? 6 - v : null, v => validCode(v, 1, 5) ? 1.2 : null), continuousSpec("CC16_302", L("CC16_302"), "ZS", "position", "worse economy -> more zero-sum", 0.45, v => fivePoint(v), v => validCode(v, 1, 5) ? 1.1 : null), continuousSpec("CC16_303", L("CC16_303"), "ONT_H", "position", "household income increased -> human optimism; decreased -> pessimism", 0.35, v => validCode(v, 1, 5) ? 6 - v : null, v => validCode(v, 1, 5) ? 0.8 : null), continuousSpec("CC16_304", L("CC16_304"), "ONT_H", "position", "expected income improvement -> optimism", 0.35, v => validCode(v, 1, 5) ? 6 - v : null, v => validCode(v, 1, 5) ? 0.8 : null));
    const salienceVars = [
    ...
    specs.push(continuousSpec("CC16_351K", L("CC16_351K"), "MAT", "position", "for minimum wage -> redistribution/labor", 1.2, v => binaryPolicyPos(v, 1.4, 4.3), v => binarySal(v, 2.0)), continuousSpec("CC16_351I", L("CC16_351I"), "MAT", "position", "for ACA repeal -> market/right; against repeal -> welfare-state support", 1.0, v => binaryPolicyPos(v, 4.2, 1.8), v => binarySal(v, 1.8)), continuousSpec("CC16_426_1", L("CC16_426_1"), "MAT", "position", "increase welfare spending -> MAT 1; decrease -> MAT 5", 0.9, v => fivePoint(v), v => validCode(v, 1, 5) ? 1.7 : null), continuousSpec("CC16_426_2", L("CC16_426_2"), "MAT", "position", "increase health spending -> MAT 1; decrease -> MAT 5", 0.75, v => fivePoint(v), v => validCode(v, 1, 5) ? 1.4 : null), continuousSpec("CC16_426_3", L("CC16_426_3"), "MAT", "position", "increase education spending -> MAT 1; decrease -> MAT 5", 0.65, v => fivePoint(v), v => validCode(v, 1, 5) ? 1.2 : null), continuousSpec("CC16_426_4", L("CC16_426_4"), "ONT_S", "position", "increase law enforcement spending -> institutional trust/order", 0.45, v => validCode(v, 1, 5) ? 6 - v : null, v => validCode(v, 1, 5) ? 0.9 : null), continuousSpec("CC16_415r", L("CC16_415r"), "MAT", "position", "deficit tradeoff: spending cuts vs tax increases", 0.7, v => validCode(v, 1, 100) ? clamp(1 + (v / 100) * 4, 1, 5) : null, v => validCode(v, 1, 100) ? 1.2 : null, "CCES slider-style variable; lower values treated as tax-increase preference."));
    const immigration = [
    ...
    specs.push(anchorSpec("CC16_331_2", L("CC16_331_2"), "national", "support border patrol -> national anchor", 0.45, v => v === 1), anchorSpec("CC16_331_5", L("CC16_331_5"), "national", "support refugee ban -> national anchor", 0.5, v => v === 1), anchorSpec("CC16_331_8", L("CC16_331_8"), "religious", "support Muslim ban -> religious/cultural boundary anchor", 0.45, v => v === 1), anchorSpec("CC16_331_1", L("CC16_331_1"), "global", "support legalization -> global/pluralist anchor", 0.3, v => v === 1));
    specs.push(continuousSpec("CC16_422c", L("CC16_422c"), "CU", "position", "agree racism exists -> pluralist/race-conscious", 1.0, v => invertFive(v), v => validCode(v, 1, 5) ? 1.6 : null), continuousSpec("CC16_422d", L("CC16_422d"), "CU", "position", "agree white advantage -> pluralist/race-conscious", 1.1, v => invertFive(v), v => validCode(v, 1, 5) ? 1.9 : null), continuousSpec("CC16_422e", L("CC16_422e"), "CU", "position", "fear of other races -> particularist/closed", 0.8, v => fivePoint(v), v => validCode(v, 1, 5) ? 1.4 : null), continuousSpec("CC16_422f", L("CC16_422f"), "CU", "position", "racial problems rare -> less race-conscious", 0.95, v => fivePoint(v), v => validCode(v, 1, 5) ? 1.6 : null), 
    ...
    specs.push(continuousSpec("CC16_330a", L("CC16_330a"), "CD", "position", "support background checks -> culturally open/regulatory", 0.45, v => binaryPolicyPos(v, 2.0, 4.0), v => binarySal(v, 1.1)), continuousSpec("CC16_330d", L("CC16_330d"), "CD", "position", "support assault-rifle ban -> culturally open/regulatory", 0.6, v => binaryPolicyPos(v, 1.8, 4.2), v => binarySal(v, 1.4)), continuousSpec("CC16_330e", L("CC16_330e"), "CD", "position", "support easier concealed carry -> culturally closed/gun-rights", 0.6, v => binaryPolicyPos(v, 4.3, 1.8), v => binarySal(v, 1.4)), continuousSpec("CC16_307", L("CC16_307"), "ONT_S", "position", "police make respondent feel safe -> local institutional trust", 0.55, v => validCode(v, 1, 4) ? 6 - (1 + ((v - 1) / 3) * 4) : null, v => validCode(v, 1, 4) ? 1.2 : null));
    specs.push(continuousSpec("CC16_351A", L("CC16_351A"), "PRO", "position", "for Garland vote -> procedural/institutional norm", 0.55, v => binaryPolicyPos(v, 4.3, 2.2), v => binarySal(v, 1.2)), continuousSpec("CC16_351C", L("CC16_351C"), "PRO", "position", "for USA Freedom Act -> procedural civil-liberties compromise", 0.35, v => binaryPolicyPos(v, 3.8, 2.2), v => binarySal(v, 0.9)), continuousSpec("CC16_351B", L("CC16_351B"), "CU", "position", "for TPP -> globalist/open; against -> economic nationalism", 0.55, v => binaryPolicyPos(v, 4.0, 2.2), v => binarySal(v, 1.1)), continuousSpec("CC16_351B", L("CC16_351B"), "ONT_S", "position", "against TPP -> anti-establishment trade skepticism", 0.45, v => binaryPolicyPos(v, 3.7, 2.0), v => binarySal(v, 1.0)), continuousSpec("CC16_351D", L("CC16_351D"), "MAT", "position", "for trade adjustment -> social-insurance cushion", 0.35, v => binaryPolicyPos(v, 2.0, 3.8), v => binarySal(v, 0.9)), anchorSpec("CC16_351B", L("CC16_351B"), "global", "support TPP -> global/economic-open anchor", 0.25, v => v === 1), anchorSpec("CC16_351B", L("CC16_351B"), "class", "oppose TPP -> class/economic-national anchor", 0.25, v => v === 2));
    specs.push(continuousSpec("CC16_312_1", L("CC16_312_1"), "CU", "position", "do not get involved in Syria -> particularist/isolationist", 0.45, v => binaryPolicyPos(v, 2.0, 3.4), v => binarySal(v, 1.0)), continuousSpec("CC16_312_2", L("CC16_312_2"), "MOR", "position", "send aid to refugees/affected countries -> wider moral concern", 0.5, v => binaryPolicyPos(v, 4.2, 2.4), v => binarySal(v, 1.1)), continuousSpec("CC16_312_7", L("CC16_312_7"), "ZS", "position", "send significant force -> threat/zero-sum orientation", 0.45, v => binaryPolicyPos(v, 4.2, 2.2), v => binarySal(v, 1.1)), continuousSpec("CC16_414_6", L("CC16_414_6"), "CU", "position", "military to uphold UN/international law -> global/institutional", 0.5, v => binaryPolicyPos(v, 4.3, 2.3), v => binarySal(v, 1.1)), continuousSpec("CC16_414_1", L("CC16_414_1"), "ZS", "position", "military to ensure oil supply -> zero-sum/resource realism", 0.4, v => binaryPolicyPos(v, 4.3, 2.4), v => binarySal(v, 1.0)));
    specs.push(continuousSpec("newsint", L("newsint"), "ENG", "activation", "political interest -> engagement", 1.4, v => validCode(v, 1, 4) ? 6 - (1 + ((v - 1) / 3) * 4) : null, () => null), continuousSpec("votereg", L("votereg"), "ENG", "activation", "registered -> higher engagement", 0.55, v => v === 1 ? 3.8 : v === 2 ? 1.6 : null, () => null), continuousSpec("CC16_327", L("CC16_327"), "ENG", "activation", "primary participation -> higher engagement", 0.8, v => v === 1 ? 4.3 : v === 2 ? 2.3 : null, () => null), continuousSpec("CC16_417a_1", L("CC16_417a_1"), "ENG", "activation", "attended local meeting", 0.5, v => v === 1 ? 4.5 : v === 2 ? 2.8 : null, () => null), continuousSpec("CC16_417a_3", L("CC16_417a_3"), "ENG", "activation", "worked for campaign", 0.55, v => v === 1 ? 5.0 : v === 2 ? 2.8 : null, () => null), continuousSpec("CC16_417a_4", L("CC16_417a_4"), "ENG", "activation", "donated money", 0.55, v => v === 1 ? 4.6 : v === 2 ? 2.7 : null, () => null));
    specs.push(
    ...
    continuousSpec("pew_religimp", L("pew_religimp"), "MOR", "salience", "religion importance -> MOR salience (independent of position direction)", 0.75, v => validCode(v, 1, 4) ? 5 - v : null, v => validCode(v, 1, 4) ? clamp(3.2 - (v - 1), 0, 3) : null), anchorSpec("pew_bornagain", L("pew_bornagain"), "religious", "born-again -> religious anchor", 0.85, v => v === 1), anchorSpec("pew_religimp", L("pew_religimp"), "religious", "religion very important -> religious anchor", 0.65, v => v === 1));
    specs.push(continuousSpec("union", L("union"), "TRB", "activation", "current/former union -> class anchor activation", 0.5, v => v === 1 ? 4.3 : v === 2 ? 3.5 : v === 3 ? 2.2 : null, () => null), continuousSpec("unionhh", L("unionhh"), "TRB", "activation", "household union -> class anchor activation", 0.4, v => v === 1 ? 4.0 : v === 2 ? 3.3 : v === 3 ? 2.2 : null, () => null), anchorSpec("union", L("union"), "class", "union membership -> class anchor", 0.9, v => v === 1 || v === 2), anchorSpec("unionhh", L("unionhh"), "class", "union household -> class anchor", 0.65, v => v === 1 || v === 2));
    specs.push(continuousSpec("CC16_328", L("CC16_328"), "MAT", "position", "2016 primary choice: Sanders/Clinton/Trump factional signal", 0.8, primaryToMat, v => validCode(v, 1, 9) ? 1.4 : null), continuousSpec("CC16_328", L("CC16_328"), "ONT_S", "position", "primary choice: establishment vs outsider signal", 0.85, primaryToOntS, v => validCode(v, 1, 9) ? 1.5 : null), continuousSpec("CC16_328", L("CC16_328"), "COM", "position", "primary choice: revolutionary vs compromise signal", 0.6, primaryToCom, v => validCode(v, 1, 9) ? 1.0 : null), categorySpec("CC16_328", L("CC16_328"), "AES", "primary choice: Clinton technocrat, Sanders authentic/fighter, Trump fighter", 0.7, primaryToAes, v => validCode(v, 1, 9) ? 1.2 : null), anchorSpec("CC16_328", L("CC16_328"), "class", "Sanders primary vote -> class anchor", 0.55, v => v === 2), anchorSpec("CC16_328", L("CC16_328"), "national", "Trump primary vote -> national anchor", 0.65, v => v === 4));
    specs.push(anchorSpec("race", L("race"), "ethnic_racial", "nonwhite race -> ethnic/racial anchor potential", 0.75, v => v === 2 || v === 3 || v === 4 || v === 5 || v === 6 || v === 7 || v === 8), anchorSpec("race", L("race"), "mixed_none", "white race alone -> mixed/no dominant anchor fallback", 0.25, v => v === 1), anchorSpec("Hispanic_origin_5", L("Hispanic_origin_5"), "ethnic_racial", "Cuban-origin Hispanic flag -> ethnic/racial anchor", 0.4, v => v === 1), anchorSpec("sexuality", L("sexuality"), "sexual", "LGBTQ identity -> sexual identity anchor", 0.8, v => validCode(v, 2, 5)), anchorSpec("trans", L("trans"), "gender", "trans identity -> gender identity anchor", 0.8, v => v === 1), anchorSpec("gender", L("gender"), "gender", "gender identity weak prior", 0.18, v => v === 1 || v === 2));
    return specs;
}
function ideologyToEps(value) {
    if (!validCode(value, 1, 5))
        return null;
    if (value === 1)
        return [0.30, 0.12, 0.04, 0.06, 0.40, 0.08];
    if (value === 2)
        return [0.36, 0.25, 0.04, 0.06, 0.24, 0.05];
    if (value === 3)
        return [0.27, 0.36, 0.11, 0.12, 0.10, 0.04];
    if (value === 4)
        return [0.18, 0.27, 0.28, 0.18, 0.05, 0.04];
    return [0.12, 0.18, 0.36, 0.24, 0.04, 0.06];
}
function primaryToAes(value) {
    if (value === 1)
        return [0.20, 0.48, 0.04, 0.08, 0.14, 0.06]; // Clinton
    if (value === 2)
        return [0.04, 0.15, 0.03, 0.42, 0.28, 0.08]; // Sanders
    if (value === 4)
        return [0.03, 0.04, 0.02, 0.18, 0.66, 0.07]; // Trump
    if (value === 5 || value === 6 || value === 7)
        return [0.35, 0.18, 0.12, 0.12, 0.18, 0.05];
    return null;
}
function primaryToMat(value) {
    if (value === 1)
        return 2.3;
    if (value === 2)
        return 1.2;
...
function partyFusionFromObamaApproval(value, row) {
    const party = partyFromRow(row);
    if (!validCode(value, 1, 4) || !party)
        return null;
    if (party === "D")
        return value <= 2 ? 4.4 : 2.6;
    if (party === "R")
        return value >= 3 ? 4.3 : 2.4;
    return value <= 2 ? 3.0 : 2.6;
}
function pfFromPartyVars(row) {
    ...
    if (pid3 === 1)
        return demStrength === 1 ? 5.0 : 4.2;
    if (pid3 === 2)
        return repStrength === 1 ? 5.0 : 4.2;
    if (pid3 === 3 && (lean === 1 || lean === 2))
        return 3.3;
    if (pid3 === 3)
        return 2.0;
    return null;
...
function partyFromRow(row) {
    const pid7 = row.num("pid7");
    if (pid7 === 1 || pid7 === 2 || pid7 === 3)
        return "D";
    if (pid7 === 5 || pid7 === 6 || pid7 === 7)
        return "R";
    if (pid7 === 4)
        return "I";
    const pid3 = row.num("CC16_421a") ?? row.num("pid3");
...
function engagementLevelFromSignature(sig) {
...
function reportedVoteFromRow(row) {
...
function stateFromRow(row) {
    ...
    if (fips != null && STATE_FIPS_TO_PO[fips])
        return STATE_FIPS_TO_PO[fips];
...
function raceGroup(row) {
    ...
    if (race === 2)
        return "black";
    if (race === 3 || row.num("hispanic") === 1)
        return row.num("Hispanic_origin_5") === 1 ? "latino_cuban" : "latino_non_cuban";
    if (race === 4)
        return "asian";
    if (race === 5)
        return "native";
...
function educationGroup(row) {
    const educ = row.num("educ");
    if (educ == null)
        return "educ_unknown";
    if (educ <= 2)
        return "noncollege_hs_or_less";
    if (educ <= 4)
        return "some_college";
    return "college_plus";
}
function weightFromRow(row, primary, fallback) {
...
async function loadBridgeRespondents(specs, labels) {
    ...
    const weightedNonMissing = new Map();
...
function addDemographicAnchorEvidence(row, builder) {
    const race = row.num("race");
    const hispanic = row.num("hispanic");
    if (race === 2)
        builder.addAnchor("ethnic_racial", scaledEvidenceWeight("race", "TRB_ANCHOR:ethnic_racial", "anchor", 1.2));
    if (race === 3 || hispanic === 1)
        builder.addAnchor("ethnic_racial", scaledEvidenceWeight("race", "TRB_ANCHOR:ethnic_racial", "anchor", 0.95));
    if (race === 4 || race === 5 || race === 6 || race === 7 || race === 8)
        builder.addAnchor("ethnic_racial", scaledEvidenceWeight("race", "TRB_ANCHOR:ethnic_racial", "anchor", 0.65));
    if (race === 1)
        builder.addAnchor("mixed_none", scaledEvidenceWeight("race", "TRB_ANCHOR:mixed_none", "anchor", 0.18));
    if (row.num("Hispanic_origin_5") === 1)
        builder.addAnchor("ethnic_racial", scaledEvidenceWeight("Hispanic_origin_5", "TRB_ANCHOR:ethnic_racial", "anchor", 0.45));
    const gender = row.num("gender");
    if (gender === 1 || gender === 2)
        builder.addAnchor("gender", scaledEvidenceWeight("gender", "TRB_ANCHOR:gender", "anchor", 0.12));
}
function loadActualStateRows() {
    if (!existsSync(PRESIDENT_RESULTS_CSV))
        return [];
    ...
    for (const row of rows) {
        ...
        if (rec.year !== "2016" || rec.office !== "US PRESIDENT")
            continue;
        ...
        if (party === "DEMOCRAT")
            current.dem_votes += votes;
        else if (party === "REPUBLICAN")
            current.rep_votes += votes;
        else
            current.third_votes += votes;
...
class VoteAccumulator {
    ...
    add(bucket, weight) {
        if (weight <= 0 || !Number.isFinite(weight))
            return;
        this.total += weight;
        if (bucket === "Democratic")
            this.democratic += weight;
        else if (bucket === "Republican")
            this.republican += weight;
        else if (bucket === "Johnson")
            this.johnson += weight;
        else if (bucket === "Stein")
            this.stein += weight;
        else if (bucket === "Other")
            this.other += weight;
        else
            this.abstainUnknown += weight;
    }
    addProbs(probs, weight) {
        for (const bucket of Object.keys(probs))
            this.add(bucket, probs[bucket] * weight);
    }
    third() {
...
class AccuracyAccumulator {
    ...
    logLoss = 0;
    brier = 0;
    add(actual, predicted, probs, weight) {
        if (actual === "AbstainUnknown" || weight <= 0)
            return;
        this.weightedN += weight;
        if (actual === predicted)
            this.top1Correct += weight;
        if (majorBucket(actual) === majorBucket(predicted))
            this.partyBucketCorrect += weight;
        const p = clamp(probs[actual] ?? 0.000001, 0.000001, 0.999999);
        this.logLoss += -Math.log(p) * weight;
        for (const b of ["Democratic", "Republican", "Johnson", "Stein", "Other"]) {
...
function majorBucket(bucket) {
    if (bucket === "Democratic")
        return "D";
    if (bucket === "Republican")
        return "R";
    if (bucket === "AbstainUnknown")
        return "A";
    return "T";
}
function candidateToBucket(name, party) {
    if (party === "Democratic")
        return "Democratic";
    if (party === "Republican")
        return "Republican";
...
function predictionToProbs(prediction, method, alphas) {
    const probs = {
        Democratic: 0,
        Republican: 0,
        Johnson: 0,
    ...
    if ((method === "calibrated_softmax_t1.5" || method === "state_calibrated_softmax_t1.5") && alphas) {
        for (let i = 0; i < prediction.candidates.length; i++) {
            ...
            if (bucket !== "AbstainUnknown")
                exps[i] = Math.exp((alphas[bucket] ?? 0) - candidate.distance / t);
        }
    }
...
function voteProbsForRespondent(respondent, candidates, method, alphas, stateAlphas) {
    const ctx = getContext(2016);
    if (!ctx)
        throw new Error("Missing 2016 context");
    const prediction = predictVote(respondent.signature, candidates, ctx, respondent.engagement, respondent.partyID, respondent.anchorDist, null, true, null);
...
function fitCalibratedSoftmaxAlphas(respondents, candidates, reported) {
...
function fitCalibratedSoftmaxAlphasToTargets(respondents, candidates, targets) {
    const ctx = getContext(2016);
    if (!ctx)
        throw new Error("Missing 2016 context");
    ...
    for (let iter = 0; iter < 80; iter++) {
        ...
        for (const respondent of respondents) {
            if (!respondent.isReportedVoter || respondent.weightVoter <= 0)
                continue;
            const prediction = predictVote(respondent.signature, candidates, ctx, respondent.engagement, respondent.partyID, respondent.anchorDist, null, true, null);
        ...
        for (const bucket of Object.keys(alphas)) {
        ...
        for (const bucket of Object.keys(alphas))
            alphas[bucket] -= meanAlpha;
    }
    return alphas;
}
function fitStateCalibratedSoftmaxAlphas(respondents, candidates, actualRows, reported) {
    const byState = new Map();
    for (const respondent of respondents) {
        if (!respondent.isReportedVoter || respondent.weightVoter <= 0)
            continue;
...
function makeSteinCandidate(base) {
    return {
        ...base,
        name: "Stein",
        party: "Independent",
...
function makeOtherCandidate(base) {
    return {
        ...base,
        name: "Other",
        party: "Independent",
...
function applyCandidateVariant(candidates, variant) {
...
function candidatesForScenario(scenario, variant) {
...
function runScenario(respondents, scenario, method, candidateVariant, mode, alphas, stateAlphas, turnoutModel, turnoutSettings) {
    ...
    for (const respondent of respondents) {
        if (mode === "fixed_voter" && (!respondent.isReportedVoter || respondent.weightVoter <= 0))
            continue;
        if (mode === "elastic_turnout" && (!turnoutModel || respondent.weightAdult <= 0))
            continue;
        ...
        if (mode === "elastic_turnout" && turnoutModel) {
            ...
            weight = respondent.weightAdult * scenarioTurnoutProbability(respondent, scenario, baseTurnout, probs, baselineProbs, settings);
        }
        ...
        if (accuracy)
            accuracy.add(respondent.reportedVote, topBucket(probs), probs, weight);
    }
...
function reportedVoteAccumulators(respondents) {
    const national = new VoteAccumulator();
    const states = new Map();
    const byGroup = new Map();
    for (const r of respondents) {
        if (!r.isReportedVoter || r.weightVoter <= 0)
            continue;
...
function turnoutFeatureMap(respondent) {
    ...
    for (const node of CONTINUOUS_NODES) {
        ...
        if (!isSelfNodeId(node))
            features.set(`node:${node}:sal`, (entry?.sal ?? priorSal(node)) / 3);
    }
    for (const node of CATEGORICAL_NODES) {
        const entry = respondent.signature[node];
        const dist = entry?.catDist ?? Array(6).fill(1 / 6);
        for (let i = 0; i < 6; i++)
            features.set(`cat:${node}:${i}`, dist[i] ?? 0);
...
function buildTurnoutExamples(respondents) {
...
function sparseDot(coefficients, example) {
    let z = 0;
    for (let i = 0; i < example.indices.length; i++)
        z += coefficients[example.indices[i]] * example.values[i];
    return z;
}
function trainTurnoutModel(respondents) {
...
function predictTurnout(model, respondent) {
    ...
    for (const [key, value] of map) {
        ...
        if (idx != null)
            z += model.coefficients[idx] * value;
    }
    return sigmoid(z);
}
function turnoutMetrics(model, respondents, split) {
    let weightedN = 0;
    let actual = 0;
    let predicted = 0;
    let logLossSum = 0;
    ...
    let correct = 0;
    for (const respondent of respondents) {
        if (respondent.weightAdult <= 0 || (split !== "all" && respondent.turnoutSplit !== split))
            continue;
...
function turnoutValidationRows(model, respondents) {
    return ["train", "holdout", "all"].map((split) => {
...
function turnoutCalibrationRows(model, respondents) {
    const rows = [];
...
function turnoutFeatureWeightRows(model) {
...
function defaultTurnoutSettings(scenario) {
...
function scenarioTurnoutProbability(respondent, scenario, baseProbability, scenarioProbs, baselineProbs, settings) {
    if (scenario === "baseline_clinton")
        return baseProbability;
    const demDelta = scenarioProbs.Democratic - baselineProbs.Democratic;
    let delta = settings.affinityCoefficient * demDelta;
    if (respondent.raceGroup === "black")
        delta += settings.blackPenalty;
    if (respondent.ageGroup === "18_29")
        delta += settings.youngBoost;
    if (respondent.raceGroup === "latino_non_cuban")
        delta += settings.latinoBoost;
    if (respondent.raceGroup === "latino_cuban") {
        delta += respondent.state === "FL" ? settings.flCubanPenalty : settings.latinoBoost * 0.4;
    }
    if (respondent.educationGroup === "college_plus" && respondent.partyID !== "D")
        delta += settings.collegePlusPenalty;
    return clamp(sigmoid(logit(baseProbability) + delta), 0.02, 0.98);
}
function nodeDistributionRows(respondents) {
    const rows = [];
...
function validationNationalRows(reported, runs) {
    return runs
...
function stateValidationRows(reportedStates, baselineRuns, actualRows) {
    const actualByState = new Map(actualRows.map((r) => [r.state_po, r]));
    const rows = [];
...
function counterfactualNationalRows(runs) {
...
function counterfactualStateRows(runs, actualRows) {
    const rows = [];
    const actualByState = new Map(actualRows.map((r) => [r.state_po, r]));
    for (const run of runs) {
        if (run.scenario === "baseline_clinton")
            continue;
...
function evSummaryRows(stateRows) {
...
function sampledTurnoutSettings(scenario, rng, band = "narrow") {
...
function evFromStateAccumulators(states) {
    let demEv = 0;
    let repEv = 0;
...
function bootstrapElasticTurnoutRows(respondents, turnoutModel, candidateVariant, alphas, stateAlphas, reps = 160, band = "narrow") {
    const method = "state_calibrated_softmax_t1.5";
    const scenarios = ["sanders_static", "sanders_perceived"];
    ...
    for (let rep = 0; rep < reps; rep++) {
        for (const scenario of scenarios) {
            ...
            for (let i = 0; i < respondents.length; i++) {
                ...
                if (respondent.weightAdult <= 0)
                    continue;
                const probs = probsForScenario[i];
                const weight = respondent.weightAdult * scenarioTurnoutProbability(respondent, scenario, baseTurnout[i], probs, baselineProbs[i], settings);
                national.addProbs(probs, weight);
                mapAccumulator(states, respondent.state).addProbs(probs, weight);
            }
...
function bootstrapSummaryRows(rows) {
...
function runCandidateSet(respondents, candidates, method, candidateVariant, mode, alphas, stateAlphas, turnoutModel, turnoutSettings, turnoutScenario = "sanders_static") {
    const baselineCandidates = candidatesForScenario("baseline_clinton", candidateVariant);
    const national = new VoteAccumulator();
    const states = new Map();
    const byGroup = new Map();
    ...
    for (const respondent of respondents) {
        ...
        if (mode === "elastic_turnout" && turnoutModel) {
            ...
            weight = respondent.weightAdult * scenarioTurnoutProbability(respondent, turnoutScenario, predictTurnout(turnoutModel, respondent), probs, baselineProbs, settings);
        }
        national.addProbs(probs, weight);
        mapAccumulator(states, respondent.state).addProbs(probs, weight);
        mapAccumulator(byGroup, `${respondent.raceGroup}|${respondent.ageGroup}|${respondent.educationGroup}`).addProbs(probs, weight);
...
function candidatesWithDemocraticReplacement(profile, variant) {
    return candidatesForScenario("baseline_clinton", variant).map((candidate) => candidate.name === "H. Clinton" ? applyCandidateVariant([{ ...profile, name: "Sanders", party: "Democratic", year: 2016 }], variant)[0] : candidate);
}
function sandersProfileWithOverrides(overrides) {
    return { ...sandersProfile("static"), ...overrides, name: "Sanders", party: "Democratic", year: 2016 };
}
function preferredOutcomeRowsForRespondents(respondents, actualStates, experiment, notes) {
    const candidateVariant = "canonical";
    const method = "state_calibrated_softmax_t1.5";
    ...
    for (const mode of ["fixed_voter", "elastic_turnout"]) {
        baselineRuns.set(mode, runScenario(respondents, "baseline_clinton", method, candidateVariant, mode, alphas, stateAlphas, turnoutModel, defaultTurnoutSettings("baseline_clinton")));
    }
    const rows = [];
    for (const scenario of ["sanders_static", "sanders_perceived"]) {
        for (const mode of ["fixed_voter", "elastic_turnout"]) {
            const run = runScenario(respondents, scenario, method, candidateVariant, mode, alphas, stateAlphas, turnoutModel, defaultTurnoutSettings(scenario));
...
async function crosswalkStressRows(labels, actualStates, baselineRespondents) {
    const rows = [
...
function fitStateCalibratedSoftmaxAlphasWithHoldout(respondents, candidates, actualRows, reported, heldoutStates) {
    const byState = new Map();
    for (const respondent of respondents) {
        if (!respondent.isReportedVoter || respondent.weightVoter <= 0)
            continue;
...
function stateCalibrationHoldoutRows(respondents, actualStates) {
    const candidateVariant = "canonical";
    const method = "state_calibrated_softmax_t1.5";
    const reported = reportedVoteAccumulators(respondents);
...
function candidateProfileGridRows(respondents, actualStates, turnoutModel, alphas, stateAlphas, candidateVariant) {
    const method = "state_calibrated_softmax_t1.5";
    const baselineByMode = new Map();
    const baselineCandidates = candidatesForScenario("baseline_clinton", candidateVariant);
    for (const mode of ["fixed_voter", "elastic_turnout"]) {
        baselineByMode.set(mode, runCandidateSet(respondents, baselineCandidates, method, candidateVariant, mode, alphas, stateAlphas, turnoutModel, defaultTurnoutSettings("baseline_clinton"), "baseline_clinton"));
    }
    const rows = [];
        ...
        for (const PRO of [1.5, 2, 2.5]) {
            for (const PF of [3.5, 4.5, 5]) {
                for (const AES of [3, 4]) {
                    ...
                    for (const mode of ["fixed_voter", "elastic_turnout"]) {
                        const run = runCandidateSet(respondents, candidates, method, candidateVariant, mode, alphas, stateAlphas, turnoutModel, defaultTurnoutSettings("sanders_static"), "sanders_static");
...
function candidateGridSummaryRows(rows) {
...
function coalitionRows(runs, reportedGroups) {
    const rows = [];
    for (const run of runs) {
        const base = runs.find((r) => r.scenario === "baseline_clinton" && r.method === run.method && r.candidateVariant === run.candidateVariant && r.mode === run.mode);
...
function respondentSampleRows(respondents) {
...
function reportMarkdown(params) {
    ...
    return [
        ...
        "|---|---|---:|---:|---:|---:|---:|---:|---:|",
        ...params.validationRows.filter((r) => r.mode === "fixed_voter").map((r) => `| ${r.candidate_variant} | ${r.method} | ${r.model_dem_share}% | ${r.model_rep_share}% | ${r.model_third_share}% | ${r.model_dem_two_party}% | ${r.model_minus_reported_dem_two_party} | ${r.accuracy_party_bucket}% | ${r.log_loss} |`),
        "",
        "## Sanders Swap",
        "",
        ...
        "|---|---|---|---:|---:|---:|---:|---:|",
        ...nationalCounter.map((r) => `| ${r.candidate_variant} | ${r.mode} | ${r.scenario} | ${r.model_dem_share}% | ${r.model_rep_share}% | ${r.model_third_share}% | ${r.model_dem_two_party}% | ${r.dem_two_party_delta_vs_clinton} |`),
        "",
        ...
        "|---|---|---|---:|---:|---:|---:|---:|---:|---|",
        ...params.bootstrapSummaryRows.map((r) => `| ${r.candidate_variant} | ${r.scenario} | ${r.band} | ${r.reps} | ${r.dem_win_rate}% | ${r.median_dem_ev} | ${r.p05_dem_ev}/${r.p95_dem_ev} | ${r.median_national_dem_two_party}% | ${r.p05_national_dem_two_party}/${r.p95_national_dem_two_party} | ${r.most_common_tipping_states} |`),
        "",
        ...
        "|---|---:|---:|---:|---:|",
        ...params.nodeRows.map((r) => `| ${r.node} | ${r.adult_weighted_mean_pos} | ${r.adult_weighted_mean_sal} | ${r.adult_weighted_mean_touches} | ${r.adult_weighted_mean_sd} |`),
        "",
        "## Important Limits",
        "",
...
async function main() {
    ...
    const scenarios = ["baseline_clinton", "sanders_static", "sanders_perceived"];
    const candidateVariants = ["canonical", "perceived_2016_patch"];
    const methods = [
        "wta",
        "softmax_t1.0",
        "softmax_t1.5",
    ...
    const stateCalibrationRows = [];
    const bootstrapRows = [];
    const candidateGridRowsAll = [];
    for (const candidateVariant of candidateVariants) {
        ...
        for (const scenario of scenarios) {
            for (const method of methods) {
                runs.push(runScenario(respondents, scenario, method, candidateVariant, "fixed_voter", method === "calibrated_softmax_t1.5" || method === "state_calibrated_softmax_t1.5" ? alphas : undefined, method === "state_calibrated_softmax_t1.5" ? stateAlphas : undefined));
            }
            runs.push(runScenario(respondents, scenario, "state_calibrated_softmax_t1.5", candidateVariant, "elastic_turnout", alphas, stateAlphas, turnoutModel, defaultTurnoutSettings(scenario)));
        }
...
//# sourceMappingURL=cces2016-electorate-bridge.js.map