...
const baselineMisses = new Set(baseline.misses.map((m) => m.archetypeId));
const v1Misses = new Set(v1.misses.map((m) => m.archetypeId));
const v2Misses = new Set(v2.misses.map((m) => m.archetypeId));
...
const comparison = {
    ...
    perMissTable: allMissIds.map((id) => {
        const bRank = baseline.allResults.find((r) => r.archetypeId === id)?.rank;
        const v1Rank = v1.allResults.find((r) => r.archetypeId === id)?.rank;
        const v2Rank = v2.allResults.find((r) => r.archetypeId === id)?.rank;
        const miss = baseline.misses.find((m) => m.archetypeId === id)
            || v1.misses.find((m) => m.archetypeId === id)
            || v2.misses.find((m) => m.archetypeId === id);
...
//# sourceMappingURL=compare-scoring.js.map