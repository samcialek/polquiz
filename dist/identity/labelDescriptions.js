...
function lookupIconic(label) {
    ...
    for (const prefix of STRIP_PREFIXES) {
        if (label.startsWith(prefix)) {
            ...
            if (tail in LABEL_DESCRIPTIONS)
                return LABEL_DESCRIPTIONS[tail];
        }
    }
    return undefined;
}
...
function atomFragment(t) {
    if (t.node === "EPS")
        return EPS_ATOMS[t.token];
    if (t.node === "AES")
        return AES_ATOMS[t.token];
    if (t.node === "MORAL_CIRCLE")
        return MORAL_CIRCLE_ATOMS[t.token];
    const node = POSITION_ATOMS[t.node];
    if (!node)
        return undefined;
    return node[t.bin];
}
export function composeAtomFallback(tokens) {
 ...
 */
export function composeArchetypeDescription(label, tokens) {
    const iconic = lookupIconic(label);
    if (iconic)
        return iconic;
    return composeAtomFallback(tokens);
}
//# sourceMappingURL=labelDescriptions.js.map