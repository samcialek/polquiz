...
import { applySingleChoiceAnswer, applySliderAnswer, applyRankingAnswer, applyAllocationAnswer, applyPairwiseAnswer, } from "../engine/update.js";
...
function auditQuestion(q, bank) {
    ...
    for (const t of q.touchProfile) {
        const nodeKey = t.node === "TRB_ANCHOR" ? "TRB_ANCHOR" : t.node;
        if (!declaredRoles[nodeKey])
            declaredRoles[nodeKey] = [];
    ...
    for (const [nodeKey, roles] of Object.entries(declaredRoles)) {
        if (nodeKey === "TRB_ANCHOR") {
            if (roles.includes("anchor") && trbInter < EPS)
                flags.push(`hollow_touch:TRB_ANCHOR`);
            continue;
        }
    ...
    if (variants.length === 0)
        flags.push("no_variants_generable");
    // Collapsed options: multiple variants but inter-variant JSD near zero everywhere
    if (variants.length > 1) {
        const maxInter = Math.max(trbInter, ...CONTINUOUS_NODES.map((n) => Math.max(posJsd[n].interVariantMaxJsd, salJsd[n].interVariantMaxJsd)), ...CATEGORICAL_NODES.map((n) => Math.max(catJsd[n].interVariantMaxJsd, salJsd[n].interVariantMaxJsd)));
        if (maxInter < EPS)
            flags.push("collapsed_variants");
    }
    return {
        bank, id: q.id,
...
const bankNames = [
    ["representative", REPRESENTATIVE_QUESTIONS],
    ["full", FULL_QUESTIONS],
];
const allAudits = [];
for (const [bank, bankQs] of bankNames) {
    ...
    for (const q of bankQs) {
        try {
            ...
            console.error(`Error auditing Q${q.id} (${q.promptShort}):`, e.message);
        }
    }
}
// JSONL per-question
...
function headlineOf(a) {
    let mx = 0;
    for (const [n, ns] of Object.entries(a.posJsd))
        if (ns.declared)
            mx = Math.max(mx, ns.interVariantMaxJsd);
    for (const [n, ns] of Object.entries(a.salJsd))
        if (ns.declared)
            mx = Math.max(mx, ns.interVariantMaxJsd);
    for (const [n, ns] of Object.entries(a.catJsd))
        if (ns.declared)
            mx = Math.max(mx, ns.interVariantMaxJsd);
    if (a.touchProfile.some((t) => t.node === "TRB_ANCHOR"))
        mx = Math.max(mx, a.trbAnchorJsd.interVariantMax);
    return mx;
}
