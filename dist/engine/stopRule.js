...
export function shouldStop(state, archetypes) {
    const cfg = getConfig();
    ...
    const adaptiveThreshold = Math.max(0.10, Math.min(cfg.STOP_POSTERIOR_THRESHOLD, 1.0 / Math.sqrt(significantCount) * 0.55));
    ...
    const anyContinuousBlocking = Object.entries(state.continuous).some(([nodeId, n]) => n.status === "live_unresolved" && !topKAgreeOnContinuous(nodeId));
    const anyCategoricalBlocking = Object.entries(state.categorical).some(([nodeId, n]) => {
        if (n.status !== "live_unresolved")
            return false;
        const sorted = [...n.catDist].sort((a, b) => b - a);
        const gap = (sorted[0] ?? 0) - (sorted[1] ?? 0);
        if (gap >= CAT_CONFIDENT_GAP)
            return false;
        return !topKAgreeOnCategorical(nodeId);
    });
    const highConfOverride = top >= cfg.HC_POSTERIOR &&
        margin >= cfg.HC_MARGIN &&
        consecutiveCount >= cfg.HC_CONSECUTIVE &&
        pairSim < cfg.HC_COSINE_BLOCK;
    const primaryStop = top >= adaptiveThreshold &&
    ...
    const secondaryStop = nAnswered >= cfg.SECONDARY_MIN_Q &&
        top >= adaptiveThreshold &&
        solidAbsMargin &&
        solidRelMargin &&
        deepStableLeader;
    const ultraConfStop = nAnswered >= cfg.UC_MIN_Q &&
        top >= cfg.UC_POSTERIOR &&
        margin >= cfg.UC_MARGIN &&
        consecutiveCount >= cfg.UC_CONSECUTIVE;
    const lateGameStop = nAnswered >= cfg.LATE_GAME_MIN_Q &&
...
//# sourceMappingURL=stopRule.js.map