 ...
 * contribution.
 */
export function moralCircleDistance(state, archetype) {
    ...
    for (const scope of MORAL_CIRCLE_SCOPES_ORDER) {
        ...
        if (archScoped === null || archScoped === undefined)
            continue;
    ...
    for (const scope of MORAL_CIRCLE_SCOPES_ORDER) {
        ...
        if (archScoped === null || archScoped === undefined)
            continue;
        if (archScoped > archetype.universalAffinity)
            archActive.add(scope);
    }
    let intersect = 0;
    let union = 0;
    for (const scope of MORAL_CIRCLE_SCOPES_ORDER) {
