 ...
 */
import { MEANINGFUL_POSITION_WEIGHT, MIN_POSITION_TOUCHES_PER_TOP_K, POSITION_DRILL_SAL_FLOOR, TOP_K_BASE, TOP_K_CLOSE_THRESHOLD, } from "./config.js";
const SCORING_NODES = [
 ...
 */
export function expectedSalience(state, nodeId) {
 ...
 */
export function getTopSalientNodes(state, base = TOP_K_BASE, closeThreshold = TOP_K_CLOSE_THRESHOLD, floor = POSITION_DRILL_SAL_FLOOR) {
 ...
 * counts as one touch.
 */
export function meaningfulPositionTouchCount(state, nodeId, questionsById, threshold = MEANINGFUL_POSITION_WEIGHT) {
    let count = 0;
 ...
 */
export function topKNodesStillNeedingDrill(state, topK, questionsById) {
    return topK.filter(n => meaningfulPositionTouchCount(state, n, questionsById) < MIN_POSITION_TOUCHES_PER_TOP_K);
}
/**
 * True iff every top-K node has reached the minimum drill depth.
 */
export function topKDrillSatisfied(state, topK, questionsById) {
 ...
 */
export function selectTopKDrillQuestion(state, available, questionsById, topK) {
    ...
    if (needsDrillSet.has("CD")) {
        ...
        if (q3) {
            const hasCleanCdPositionTouch = q3.touchProfile?.some(tp => tp.node === "CD" &&
                tp.role === "position" &&
                tp.weight >= MEANINGFUL_POSITION_WEIGHT);
            if (hasCleanCdPositionTouch)
                return q3;
        }
    }
    const candidates = [];
    ...
    if (candidates.length === 0)
        return null;
    candidates.sort((a, b) => {
        if (b.multiHit !== a.multiHit)
            return b.multiHit - a.multiHit;
        return b.quality - a.quality;
    });
    return candidates[0].q;
}
//# sourceMappingURL=topKDrill.js.map