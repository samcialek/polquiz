...
// ---------------------------------------------------------------------------
const LIVE_OR_UNRESOLVED = new Set(["unknown", "live_unresolved"]);
...
function evaluatePredicate(state, predicate) {
    ...
    switch (predicate) {
        ...
        case "q102_religion_supported": {
            ...
            if (!q102Ans)
                return false;
            return (q102Ans.supportHigh?.includes("religion") ?? false)
                || (q102Ans.supportMid?.includes("religion") ?? false);
        }
        default:
 ...
 */
export function isQuestionEligible(state, q) {
...
// ---------------------------------------------------------------------------
function scoreSaliencePhase(state, q, _archetypes) {
    let salienceBoost = 0;
    let coverageScore = 0;
    let totalTouches = 0;
 ...
 */
export function pruneByRespondentSalience(state, archetypes, threshold = 0.15) {
...
function scorePruneDiscriminatePhase(state, q, archetypes) {
    ...
    return ((discrimScore * 2.0 + coverage + uncertainty * 0.5) *
        q.quality *
...
function scoreInformationGain(state, q, archetypes) {
    ...
    return ((discrimScore * 2.5 + cfg.DEVILS_ADVOCATE_WEIGHT * devilScore) *
        entropyWeight *
        q.quality *
        (q.rewriteNeeded ? 0.7 : 1.0));
}
...
function scorePairwiseDiscrimination(state, q, archetypes) {
    ...
    const differentiatingNodes = new Map(); // node → difference magnitude
    for (const nodeId of CONTINUOUS_NODES) {
    ...
    for (const nodeId of CATEGORICAL_NODES) {
        const t1 = leader.nodes[nodeId];
        const t2 = rival.nodes[nodeId];
        if (!t1 || !t2 || t1.kind !== "categorical" || t2.kind !== "categorical")
            continue;
        let dot = 0;
        for (let i = 0; i < 6; i++) {
            dot += (t1.probs[i] ?? 0) * (t2.probs[i] ?? 0);
    ...
    if (differentiatingNodes.size === 0)
        return 0;
...
// Question selection
// ---------------------------------------------------------------------------
export function selectNextQuestion(state, available, archetypes) {
    const eligible = available.filter((q) => !(q.id in state.answers) && isQuestionEligible(state, q));
    if (!eligible.length)
        return null;
...
export function scoreQuestionForExposure(state, q, archetypes) {
...
export function selectNextBatch(state, available, archetypes) {
