...
const ITEM_A = {
    ...
    uiType: "slider",
    prompt: "How much moral concern should politics give to any human being, regardless of where they live or which group they belong to?",
    scaleMin: 0,
    scaleMax: 100,
    scaleAnchors: SHARED_ANCHORS,
    target: "universalAffinity",
};
const SCOPED_ROW_LABELS = {
    national: {
...
const ITEM_B = {
    ...
    uiType: "scoped_grid",
    prompt: "Using the same scale, how much extra moral concern should politics give to each group when decisions affect them?",
    scaleMin: 0,
    scaleMax: 100,
    scaleAnchors: SHARED_ANCHORS,
    notMeaningfulOption: {
...
export const MORAL_CIRCLE_BATTERY_SPEC = {
...
function numericVector(input) {
    ...
    const out = [input.universalAffinity];
 ...
 */
export function detectMoralCircleSatisficingFlags(input) {
    const flags = [];
    ...
    for (const scope of MORAL_CIRCLE_SCOPES) {
        ...
        if (v > u)
            scopedAtOrBelowUniversal = false;
    }
...
//# sourceMappingURL=moralCircleCalibrationBattery.js.map