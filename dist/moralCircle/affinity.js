...
export const MORAL_CIRCLE_SCOPES = [
    "national",
    "religious",
    "ethnic_racial",
    "class",
...
export function isMoralCircleScope(value) {
    return typeof value === "string" && MORAL_CIRCLE_SCOPES.includes(value);
}
/** Clamp a number to [0, 100]; non-finite → 0. */
export function clampAffinity100(value) {
 ...
 */
export function coerceScopedAffinityForLegacy(scoped, universalAffinity) {
    if (scoped === null || scoped === undefined)
        return clampAffinity100(universalAffinity);
 ...
 */
export function computeExcessAffinities(universalAffinity, scopedAffinities) {
 ...
 */
export function getActiveMoralCircleBoundaries(excessAffinities, threshold = MORAL_CIRCLE_REPORT_ACTIVE_THRESHOLD) {
    const out = [];
    for (const scope of MORAL_CIRCLE_SCOPES) {
 ...
 */
export function computeMoralCircleIntensity(excessAffinities) {
    let sumSq = 0;
    for (const scope of MORAL_CIRCLE_SCOPES) {
        const e = excessAffinities[scope];
        sumSq += e * e;
 ...
 */
export function deriveMoralCircleAffinity(input) {
 ...
 * module.
 */
export function validateMoralCircleAffinityInput(input) {
    const issues = [];
    ...
    const sa = input.scopedAffinities;
    for (const scope of MORAL_CIRCLE_SCOPES) {
        ...
        if (v === null)
            continue; // explicit null is valid "not meaningful to me"
